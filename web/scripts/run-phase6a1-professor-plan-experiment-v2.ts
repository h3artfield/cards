#!/usr/bin/env npx tsx
/**
 * Phase 6A.1 — Sealed 28-case Professor PLAN experiment v2.
...
const OUT_DIR = resolve("data/milestones/deck-synthesis/phase6a1-professor-plan-experiment-v2");
const SMOKE_DIR = resolve("data/milestones/deck-synthesis/phase6a1-professor-plan-experiment-v2-frontier-smoke");
const CASES_DIR = resolve(OUT_DIR, "cases");
const MANIFEST_PATH = resolve(OUT_DIR, "phase6a1-professor-plan-experiment-manifest-v2.json");
const SEALED_PATH = resolve(OUT_DIR, "phase6a1-professor-plan-experiment-sealed-v2.json");
const REPORT_PATH = resolve(OUT_DIR, "phase6a1-professor-plan-experiment-report-v2.json");
const RUN_HISTORY_DIR = resolve(
  "data/milestones/deck-synthesis/phase6a1-professor-plan-experiment-v2-run-history",
);
...
async function runFormalExperiment(modelPin: ReturnType<typeof loadProfessorModelPinV2>) {
  if (!existsSync(PROFESSOR_PLAN_STACK_FREEZE_V2_PATH)) {
    throw new Error(`Missing stack freeze ${PROFESSOR_PLAN_STACK_FREEZE_V2_PATH}. Run frontier smoke first.`);
  }
  ...
  for (const entry of entries) {
    ...
    console.log(`[${caseIndex}/${entries.length}] Professor PLAN v2 ${entry.caseId} (${entry.commanders.join(" / ")})`);
  ...
  const manifest = {
    version: "phase6a1-professor-plan-experiment-manifest-v2",
    ...
    authorization: {
      ...
      professorPlanExperimentV2: "AUTHORIZED_FORMAL_RUN",
      gateB: "WAIT",
  ...
  const sealed = {
    version: "phase6a1-professor-plan-experiment-sealed-v2",
    sealedAt,
    ...
    note: preGoldGatesClear
      ? "Formal Professor v2 outputs sealed after all 28 cases complete. REPORT AND WAIT for pre-gold audit."
      : "Interim seal after resume pass; runtime or validation gates not yet clear.",
    caseIds: caseRecords.map((c) => c.caseId),
  };

  const report = {
    version: "phase6a1-professor-plan-experiment-report-v2",
    generatedAt: sealedAt,
    overallStatus: preGoldGatesClear ? "SEALED_AWAIT_PRE_GOLD_AUDIT" : "PARTIAL_OR_BLOCKED_AWAIT_COMPLETION",
    manifestPath: "phase6a1-professor-plan-experiment-v2/phase6a1-professor-plan-experiment-manifest-v2.json",
    sealedPath: "phase6a1-professor-plan-experiment-v2/phase6a1-professor-plan-experiment-sealed-v2.json",
