#!/usr/bin/env npx tsx
/** Chatterfang prospective Professor v3 smoke runner v1 — live NO_MODEL auth; full executable stack when separately authorized. */
...
import { runProfessorV3ChatterfangProspectiveSmokeExecutionV1 } from "./lib/phase6a1-professor-v3-chatterfang-prospective-smoke-execution-v1";

loadProjectEnvLocal();

async function main() {
  const result = await runProfessorV3ChatterfangProspectiveSmokeExecutionV1({
  ...
  console.log(
    JSON.stringify(
      {
        version: "phase6a1-professor-v3-smoke-chatterfang-prospective-executed-v1",
