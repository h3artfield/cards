#!/usr/bin/env npx tsx
/**
 * Phase 5.5 — Full 168-case human mechanical + retrieval review (DEV 118 + blind-v3 50).
...
type CaseRecord = {
  caseId: string;
  set: "dev_regression" | "blind_holdout_v1" | "blind_holdout_v2" | "blind_holdout_v3";
  category: string;
  commandZoneConfiguration: string;
  commandZoneComposition?: {
    configuration: string;
    crossSupportCount: number;
...
function hasTokenOutputMultiplier(text: string): boolean {
  return (
    /twice as many tokens|double the number of .* tokens you create|create twice that many tokens|that many tokens plus one/.test(
      text,
    ) && !/double strike\b/.test(text)
  );
}
...
function inferExpectedDirection(oracleText: string, commanderNames: string[]): string {
  ...
  if (/create .* token/.test(t) && hasTokenOutputMultiplier(t)) parts.push("OUTPUT_MULTIPLIER → TOKEN_GENERATION");
  ...
  if (/choose a background/.test(t) && !/perfect clone/.test(t)) parts.push("OPTIONAL_COMMAND_ZONE_CONTEXT (Background permitted)");
...
function classifyZeroDirection(input: {
  ...
  if (
    input.evaluationContextStatus === "COMMAND_ZONE_CONTEXT_REQUIRED" ||
    input.evaluationContextStatus === "OPTIONAL_COMMAND_ZONE_CONTEXT"
  ) {
    return { classification: "N/A", note: "Command-zone context classification — not a zero-direction failure." };
  }
...
function adjudicateMechanical(input: {
  ...
  if (evaluationContextStatus === "OPTIONAL_COMMAND_ZONE_CONTEXT") {
    ...
    return {
      ...
      detail: optionalOk
        ? "Commander has complete solo mechanics; Background/Partner is optional context."
        : "OPTIONAL context flag may be incorrect.",
      humanReview: {
        ...review,
        q10_contextJustified: optionalOk,
        q1_primaryReal: !!primary,
        q6_centralMissing: !primary,
      },
    };
  }
  ...
  if (
    hasTokenOutputMultiplier(text) &&
    !primary.drivers.includes("TOKEN_GENERATION") &&
    !anchors.some((a) => a.mechanism === "TOKEN_GENERATION" || a.mechanism === "OUTPUT_MULTIPLIER")
  ) {
    if (outcome === "ACCEPTED") {
      outcome = "MISSING_CENTRAL_DIRECTION";
      rootCauseClass = "OUTPUT_MULTIPLIER_NOT_REPRESENTED";
      detail = "Token output multiplier not represented in primary direction.";
      review.q6_centralMissing = true;
    }
  }

  if (
    /\{t\}:.*create .* token|\{[^}]+\},?\s*\{t\}:.*create .* token|pay \d+ life:.*draw/.test(text) &&
    !anchors.some((a) => a.anchorKind === "ACTIVATED_ACTION") &&
    !primary.directionAnchors.some((a) => a.anchorKind === "ACTIVATED_ACTION")
  ) {
    if (outcome === "ACCEPTED") {
      outcome = "MISSING_CENTRAL_DIRECTION";
      rootCauseClass = "ACTIVATED_ACTION_NOT_ANCHORED";
      detail = "Activated ability creates/modifies state but lacks ACTIVATED_ACTION anchor.";
      review.q6_centralMissing = true;
    }
  }
...
async function main() {
  ...
  const cases = [
    ...
    ...ARCHETYPE_DISCOVERY_BLIND_HOLDOUT_V3.map((c) => ({
      ...c,
      set: "blind_holdout_v3" as const,
      category: c.category,
    })),
  ];
  ...
  for (const c of cases) {
    ...
    records.push({
      ...
      commandZoneConfiguration:
        "commandZoneConfiguration" in c
          ? (c as { commandZoneConfiguration?: string }).commandZoneConfiguration
          : c.commanders.length > 1
            ? "partner_pair"
            : "single_commander",
      commandZoneComposition: report.commandZoneComposition
        ? {
            configuration: report.commandZoneComposition.configuration,
            crossSupportCount: report.commandZoneComposition.crossSupportDirections.length,
            independentCount: report.commandZoneComposition.independentDirections.length,
          }
        : null,
  ...
  const contextIncorrect = records.filter(
    (r) => r.contextOutcome === "CONTEXT_REQUIRED_INCORRECT" || r.contextOutcome === "OPTIONAL_CONTEXT_INCORRECT",
  ).length;
  ...
  const activatedActionAccuracy =
    scorable.filter((r) => {
      const needs = /\{t\}:.*create|\{[^}]+\},?\s*\{t\}:|pay \d+ life:.*draw/.test(r.commanderOracleText.toLowerCase());
      if (!needs) return true;
      return (
        r.directionAnchors.some((a) => a.anchorKind === "ACTIVATED_ACTION") ||
        (r.primaryDirection?.directionAnchors ?? []).some((a) => a.anchorKind === "ACTIVATED_ACTION")
      );
    }).length / scorable.length;

  const outputMultiplierAccuracy =
    scorable.filter((r) => {
      if (!hasTokenOutputMultiplier(r.commanderOracleText.toLowerCase())) return true;
      return (
        r.directionAnchors.some((a) => a.mechanism === "OUTPUT_MULTIPLIER") ||
        (r.primaryDirection?.directionAnchors ?? []).some((a) => a.mechanism === "OUTPUT_MULTIPLIER")
      );
    }).length / scorable.length;

  const compositeDirectionAccuracy =
    (() => {
      const cohort = scorable.filter((r) =>
        /search .* library for a land.*exile the top/.test(r.commanderOracleText.toLowerCase()),
      );
      if (cohort.length === 0) return 1;
      return (
        cohort.filter((r) => r.primaryDirection?.mechanicalDescription?.startsWith("COMPOSITE:")).length /
        cohort.length
      );
    })();

  const partnerCompositionUsability = (() => {
    const cohort = records.filter((r) => r.commandZoneConfiguration === "partner_pair");
    if (cohort.length === 0) return 1;
    return cohort.filter((r) => r.primaryDirection && r.mechanicalOutcome === "ACCEPTED").length / cohort.length;
  })();

  const backgroundCompositionUsability = (() => {
    const cohort = records.filter((r) => r.commandZoneConfiguration === "commander_with_background");
    if (cohort.length === 0) return 1;
    return cohort.filter((r) => r.primaryDirection && r.mechanicalOutcome === "ACCEPTED").length / cohort.length;
  })();

  const report = {
    version: "archetype-discovery-v1.5.0-human-mechanical-review",
    discoveryEngineVersion: ARCHETYPE_DISCOVERY_V1_VERSION,
    causalInferenceVersion: COMMANDER_CAUSAL_INFERENCE_VERSION,
    generatedAt: new Date().toISOString(),
    reviewPolicy: {
      phase: "5.5",
      phase542Freeze: "ACCEPTED_FROZEN",
      blindV3Status: "DEVELOPMENT_DIAGNOSTIC_SPENT",
      rc8Frozen: true,
      primaryOutput: "CommanderBuildDirection + DirectionAnchor + RetrievalSpecification + COMPOSITE_DIRECTION",
      humanGroundTruth: true,
      phase541Implementation: "COMPLETE",
      phase542Implementation: "COMPLETE",
      phase54Freeze: "WAIT — DEV gate evaluation",
      blindV3: "WAIT",
    },
    ...
    metrics: {
      ...
      activatedActionAnchorAccuracy: activatedActionAccuracy,
      outputMultiplierAccuracy,
      compositeDirectionAccuracy,
      partnerCompositionUsability,
      backgroundCompositionUsability,
      contextClassification:
        contextScorable.length > 0 ? contextCorrect / contextScorable.length : 1,
      contextRequired: {
        correct: contextCorrect,
        incorrect: contextIncorrect,
        caseIds: records
          .filter((r) => r.contextOutcome === "CONTEXT_REQUIRED_CORRECT" || r.contextOutcome === "OPTIONAL_CONTEXT_CORRECT")
          .map((r) => r.caseId),
      },
      v132ToV141Regression: regressionCounts,
    },
    devGate55: {
      mechanicalPrecision: mechAccepted / scorable.length,
      primaryCorrectness:
      ...
      mechanismCorrectness:
        scorable.filter((r) => r.mechanicalOutcome !== "MECHANISM_TYPE_WRONG").length / scorable.length,
      ...
      thresholds: {
        ...
        contextClassification: 0.95,
        partnerCompositionUsability: 0.9,
        backgroundCompositionUsability: 0.9,
      },
      pass:
        mechAccepted / scorable.length >= 0.95 &&
        scorable.filter((r) => r.primaryDirection && r.mechanicalOutcome !== "MISSING_CENTRAL_DIRECTION").length /
          scorable.length >=
          0.95 &&
        scorable.filter((r) => !["CAUSAL_CHAIN_WRONG", "ANCHOR_KIND_WRONG"].includes(r.mechanicalOutcome)).length /
          scorable.length >=
          0.95 &&
        scorable.filter((r) => r.mechanicalOutcome !== "ANCHOR_KIND_WRONG").length / scorable.length >= 0.95 &&
        scorable.filter((r) => r.mechanicalOutcome !== "MECHANISM_TYPE_WRONG").length / scorable.length >= 0.95 &&
        retrAccepted / scorable.length >= 0.95 &&
        scorable.filter((r) => !r.primaryDirection).length / scorable.length <= 0.05 &&
        missingCentralCases.length / scorable.length <= 0.05 &&
        payoffOnly === 0 &&
        (contextScorable.length > 0 ? contextCorrect / contextScorable.length : 1) >= 0.95 &&
        partnerCompositionUsability >= 0.9 &&
        backgroundCompositionUsability >= 0.9,
    },
    diagnosis: {
    ...
    authorization: {
      phase54HumanReview: "ACCEPTED_AS_DEVELOPMENT_EVIDENCE",
      phase542: "COMPLETE — awaiting gate",
      phase54Freeze: "WAIT",
      blindV3: "WAIT",
      phase6: "WAIT",
  ...
  const outPath = resolve(outDir, "archetype-discovery-v1.5.0-human-mechanical-review.json");
  const matrixPath = resolve(outDir, "archetype-discovery-v1.4.1-to-v1.4.2-regression-matrix.json");
  writeFileSync(outPath, JSON.stringify(report, null, 2));
  writeFileSync(matrixPath, JSON.stringify({ version: "v1.4.2-to-v1.5.0", matrix: report.regressionMatrix, counts: regressionCounts, devGate55: report.devGate55 }, null, 2));

  console.log(JSON.stringify({ metrics: report.metrics, devGate55: report.devGate55, diagnosis: report.diagnosis, outPath, hash: createHash("sha256").update(JSON.stringify(report)).digest("hex") }, null, 2));
}
