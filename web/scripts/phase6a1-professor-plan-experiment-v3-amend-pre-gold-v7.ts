#!/usr/bin/env npx tsx
/**
 * Pre-gold amendment v7: ontology v5 destination fail-closed; graph v7; v6 symmetric layer frozen.
 */
...
import {
  ...
  PACKAGE_SYNERGY_GRAPH_V7_VERSION,
  type PackageSynergyTypedEdge,
} from "./lib/phase6a1-package-synergy-graph-v7";
import { CANONICAL_RESOURCE_ONTOLOGY_V5_VERSION } from "./lib/phase6a1-canonical-resource-ontology-v5";
...
const SPENT_AMENDED_V6_DIR = resolve(
  "data/milestones/deck-synthesis/phase6a1-professor-plan-experiment-v3-amended-v6-spent-diagnostic",
);
const SOURCE_AMENDED_V6_DIR = resolve("data/milestones/deck-synthesis/phase6a1-professor-plan-experiment-v3-amended-v6");
const AMENDED_V7_DIR = resolve("data/milestones/deck-synthesis/phase6a1-professor-plan-experiment-v3-amended-v7");
const AMENDED_V7_CASES_DIR = resolve(AMENDED_V7_DIR, "cases");
const PRE_GOLD_REAUDIT_SOURCE =
  "C:/Users/h3art/Downloads/phase6a1-professor-plan-experiment-v3-amended-v6-pre-gold-reaudit-gpt56sol-v1.json";
const EXPECTED_V7_CAUSAL_TYPED_EDGE_RECORDS = 46;
...
function recomputeThreeLens(casePath: string): void {
  ...
  record.synergyGraphVersion = PACKAGE_SYNERGY_GRAPH_V7_VERSION;
  record.canonicalResourceOntologyVersion = CANONICAL_RESOURCE_ONTOLOGY_V5_VERSION;
  ...
  record.preGoldAmendment = {
    version: "phase6a1-professor-plan-experiment-v3-amended-pre-gold-v7",
...
async function main() {
  const skipArchive = process.env.PROFESSOR_V3_AMEND_V7_SKIP_ARCHIVE === "1";

  mkdirSync(AMENDED_V7_CASES_DIR, { recursive: true });

  if (!skipArchive) {
    mkdirSync(SPENT_AMENDED_V6_DIR, { recursive: true });
    for (const name of readdirSync(SOURCE_AMENDED_V6_DIR)) {
      const src = resolve(SOURCE_AMENDED_V6_DIR, name);
      const dest = resolve(SPENT_AMENDED_V6_DIR, name);
      if (name === "cases") {
  ...
  const predecessorCasesDir = skipArchive ? resolve(SPENT_AMENDED_V6_DIR, "cases") : resolve(SPENT_AMENDED_V6_DIR, "cases");
  if (!existsSync(predecessorCasesDir)) {
    throw new Error(`Missing predecessor v6 spent cases at ${predecessorCasesDir}`);
  }
  ...
  for (const file of readdirSync(resolve(SOURCE_AMENDED_V6_DIR, "cases")).filter((f) => f.endsWith(".json"))) {
    const dest = resolve(AMENDED_V7_CASES_DIR, file);
    if (!existsSync(dest)) {
      copyFileSync(resolve(SOURCE_AMENDED_V6_DIR, "cases", file), dest);
    }
  }

  for (const file of readdirSync(AMENDED_V7_CASES_DIR).filter((f) => f.endsWith(".json"))) {
    recomputeThreeLens(resolve(AMENDED_V7_CASES_DIR, file));
  }

  const caseFiles = readdirSync(AMENDED_V7_CASES_DIR)
    .filter((f) => f.endsWith(".json"))
    .sort();
  const casePaths = caseFiles.map((f) => resolve(AMENDED_V7_CASES_DIR, f));
  ...
  const manifest = {
    version: "phase6a1-professor-plan-experiment-manifest-v3-amended-v7",
    generatedAt: new Date().toISOString(),
    sourceSpentDiagnostic: "phase6a1-professor-plan-experiment-v3-amended-v6-spent-diagnostic",
    preGoldAuditReference: "phase6a1-professor-plan-experiment-v3-amended-v6-pre-gold-reaudit-gpt56sol-v1.json",
    amendment: {
      version: "phase6a1-professor-plan-experiment-v3-amended-pre-gold-v7",
      professorRerun: false,
      canonicalResourceOntologyVersion: CANONICAL_RESOURCE_ONTOLOGY_V5_VERSION,
      synergyGraphVersion: PACKAGE_SYNERGY_GRAPH_V7_VERSION,
      ...
      recoveredCreatureDestinationFailClosed: true,
    ...
    authorization: {
      ...
      professorV3FormalRun: "AMENDED_PRE_GOLD_V7",
  ...
  const manifestPath = resolve(AMENDED_V7_DIR, "phase6a1-professor-plan-experiment-manifest-v3-amended-v7.json");
  ...
  const preGoldClear =
    ...
    manifest.population.causalTypedEdgeRecords === EXPECTED_V7_CAUSAL_TYPED_EDGE_RECORDS &&
    causalPredecessorDiff.passed &&
    manifest.population.sealedSuccessCases === 28;

  const sealed = {
    version: "phase6a1-professor-plan-experiment-sealed-v3-amended-v7",
    sealedAt,
    caseCount: caseRecords.length,
    manifestSha256,
    caseSetSha256,
    sealKind: preGoldClear ? "AMENDED_V7_AWAIT_PRE_GOLD_REAUDIT" : "AMENDED_V7_INTERIM",
    preGoldAuditStatus: preGoldClear ? "AWAITING_INDEPENDENT_REAUDIT" : "BLOCKED_VALIDATION_OR_GRAPH_QA",
    goldEvaluationStatus: "NOT_RUN",
    note: "Amendment v7: RECOVERED_CREATURE destination fail-closed; v6 symmetric layer frozen. Professor outputs unchanged.",
    provenanceSidecar: "phase6a1-professor-plan-experiment-v3-seal-provenance-sidecar-v1.json",
    predecessorAmendedPopulation: "phase6a1-professor-plan-experiment-v3-amended-v6-spent-diagnostic",
  };

  const report = {
    version: "phase6a1-professor-plan-experiment-report-v3-amended-v7",
    generatedAt: sealedAt,
    overallStatus: preGoldClear ? "AMENDED_V7_AWAIT_PRE_GOLD_REAUDIT" : "AMENDED_V7_PARTIAL_OR_BLOCKED",
    summary: manifest.population,
    causalPredecessorDiff,
    goldComparison: "KEEP_SEALED",
  };

  writeFileSync(resolve(AMENDED_V7_DIR, "phase6a1-professor-plan-experiment-sealed-v3-amended-v7.json"), JSON.stringify(sealed, null, 2));
  writeFileSync(resolve(AMENDED_V7_DIR, "phase6a1-professor-plan-experiment-report-v3-amended-v7.json"), JSON.stringify(report, null, 2));

  writeFileSync(
    resolve(AMENDED_V7_DIR, "phase6a1-professor-plan-experiment-v3-amended-v7-completion-v1.json"),
    JSON.stringify(
      {
        version: "phase6a1-professor-plan-experiment-v3-amended-v7-completion-v1",
        completedAt: sealedAt,
        status: preGoldClear ? "AMENDED_V7_AWAIT_PRE_GOLD_REAUDIT" : "AMENDED_V7_PARTIAL_OR_BLOCKED",
        predecessor: {
          version: "phase6a1-professor-plan-experiment-v3-amended-v6-completion-v1",
          status: "SPENT_DIAGNOSTIC_PRE_GOLD_BLOCK",
          archive: "phase6a1-professor-plan-experiment-v3-amended-v6-spent-diagnostic",
          preGoldReaudit: "phase6a1-professor-plan-experiment-v3-amended-v6-pre-gold-reaudit-gpt56sol-v1.json",
        ...
        repairs: {
          ...
          atomicSharedEnablerCompatibility: true,
          exhaustiveSymmetricEdgeAudit: true,
          semanticRegressionAssertions: 23,
          professorRerun: false,
        },
        population: manifest.population,
        causalPredecessorDiff,
        artifacts: {
          manifest: "phase6a1-professor-plan-experiment-manifest-v3-amended-v7.json",
          manifestSha256,
          sealed: "phase6a1-professor-plan-experiment-sealed-v3-amended-v7.json",
          caseSetSha256,
          report: "phase6a1-professor-plan-experiment-report-v3-amended-v7.json",
        },
        gateDisposition: {
          semanticRetrievalFreeze: "FROZEN",
          professorV3FormalRun: "AMENDED_PRE_GOLD_V7",
  ...
  writeFileSync(
    resolve("data/milestones/deck-synthesis/phase6a1-professor-plan-experiment-v3-amended-v6-spent-diagnostic-manifest-v1.json"),
    JSON.stringify(
      {
        version: "phase6a1-professor-plan-experiment-v3-amended-v6-spent-diagnostic-manifest-v1",
        spentAt: sealedAt,
        status: "SPENT_DIAGNOSTIC_PRE_GOLD_BLOCK",
        policy:
          "Amended v6 population preserved after re-audit BLOCK on RECOVERED_CREATURE destination compatibility. Do not use for gold comparison.",
        preGoldReaudit: "phase6a1-professor-plan-experiment-v3-amended-v6-pre-gold-reaudit-gpt56sol-v1.json",
        successor: "phase6a1-professor-plan-experiment-v3-amended-v7",
      },
      null,
      2,
    ),
  ...
  if (existsSync(PRE_GOLD_REAUDIT_SOURCE)) {
    copyFileSync(
      ...
      resolve(
        "data/milestones/deck-synthesis/phase6a1-professor-plan-experiment-v3-amended-v6-pre-gold-reaudit-gpt56sol-v1.json",
      ),
    );
  }
