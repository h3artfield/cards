#!/usr/bin/env npx tsx
/** Zada executable-stack sealing audit — 0 OpenAI calls. */
...
import {
  createProfessorV3ZadaModelRequestBoundaryFetchStubV1,
  isProfessorV3ZadaModelRequestBoundaryErrorV1,
  PROFESSOR_V3_ZADA_MODEL_REQUEST_BOUNDARY_REACHED_V1,
} from "./lib/phase6a1-professor-v3-zada-model-request-boundary-v1";
...
import {
  ...
  runProfessorV3ZadaProspectiveSmokeExecutionV1,
} from "./lib/phase6a1-professor-v3-zada-prospective-smoke-execution-v1";
import {
  assertProfessorV3CandidateExecutionAuthorizationV11,
  assertProfessorV3ZadaExecutableSmokeExecutionPreflightV11,
  loadProfessorV3ReviewedExecutionIdentityV10,
...
import {
  assertProfessorV3ZadaSmokeOutputsAbsentV1,
  resolveProfessorV3SmokeZadaProspectiveOutputTargetsV1,
...
const OUT_PATH = resolve(MILESTONES, "phase6a1-professor-v3-zada-executable-stack-sealing-audit-v1.json");
...
function withPatchedCandidateAuth() {
  const identity = loadProfessorV3ReviewedExecutionIdentityV10();
  return {
    ...PROFESSOR_V3_ZADA_PROSPECTIVE_SMOKE_CANDIDATE_EXECUTION_AUTHORIZATION_V1,
    executionIdentityArtifactSha256: sha256File(PROFESSOR_V3_SMOKE_EXECUTION_IDENTITY_V10_PATH),
...
function recordOutputAbsenceBlock(
  ...
  description: string,
  tempRoot: string,
  setup: (targets: ReturnType<typeof resolveProfessorV3SmokeZadaProspectiveOutputTargetsV1>) => void,
) {
  const targets = resolveProfessorV3SmokeZadaProspectiveOutputTargetsV1(tempRoot);
  setup(targets);
  try {
    assertProfessorV3ZadaSmokeOutputsAbsentV1(targets);
...
async function main() {
  ...
  const checks: Check[] = [];
  const runner = buildProfessorV3ZadaProspectiveRunnerBindingV1({
  ...
  try {
    ...
    record(checks, "zada-executable-preflight-pass", "Zada executable-stack identity/pins/candidate auth preflight passes", true);
  } catch (err) {
    record(checks, "zada-executable-preflight-pass", "Zada executable-stack identity/pins/candidate auth preflight passes", false, String(err));
  }
  ...
  if (preflightSealed) {
    try {
      const preModelAudit = await runProfessorV3ZadaPreModelAuditV1();
      ...
      record(
        ...
        "Zada pre-model audit with real RAG context passes all checks",
        preModelAudit.failed === 0,
        `passed=${preModelAudit.passed}; failed=${preModelAudit.failed}`,
      );
    } catch (err) {
      record(checks, "pre-model-audit-all-pass", "Zada pre-model audit with real RAG context passes all checks", false, String(err));
    }

    recordOutputAbsenceBlock(checks, "output-absence-existing-result", "Existing result artifact blocks before model request", mkdtempSync(join(tmpdir(), "zada-output-absence-")), (targets) => {
      writeFileSync(targets.result, "{}");
    });
    recordOutputAbsenceBlock(checks, "output-absence-existing-failure", "Existing failure artifact blocks before model request", mkdtempSync(join(tmpdir(), "zada-output-absence-")), (targets) => {
      writeFileSync(targets.failure, "{}");
    });
    recordOutputAbsenceBlock(checks, "output-absence-existing-manifest", "Existing manifest artifact blocks before model request", mkdtempSync(join(tmpdir(), "zada-output-absence-")), (targets) => {
      writeFileSync(targets.manifest, "{}");
    });
    recordOutputAbsenceBlock(checks, "output-absence-existing-model-attempts-dir", "Existing model-attempts directory blocks before model request", mkdtempSync(join(tmpdir(), "zada-output-absence-")), (targets) => {
      mkdirSync(targets.modelAttemptsDir, { recursive: false });
    });

    const tempRoot = mkdtempSync(join(tmpdir(), "zada-exec-boundary-"));
    let boundaryReached = false;
    let requestArtifactPresent = false;
    try {
      await runProfessorV3ZadaProspectiveSmokeExecutionV1({
        authorization: candidateAuth,
        executeSwitchPresent: true,
        milestonesDir: tempRoot,
        modelCallerOptions: {
          requireOpenAiKey: () => "stub-key-for-boundary-test",
          fetchImpl: createProfessorV3ZadaModelRequestBoundaryFetchStubV1(),
        },
      });
      record(
        checks,
      ...
      boundaryReached = isProfessorV3ZadaModelRequestBoundaryErrorV1(error);
      requestArtifactPresent = existsSync(
        join(
          tempRoot,
          "phase6a1-professor-v3-smoke-zada-prospective-model-attempts-v1/attempt-000-api-request-body.json",
        ),
      );
      record(
        checks,
        ...
        `boundaryError=${boundaryReached}; requestArtifact=${requestArtifactPresent}; stage=${PROFESSOR_V3_ZADA_MODEL_REQUEST_BOUNDARY_REACHED_V1}`,
      );
    }
    ...
    record(checks, "pre-model-audit-all-pass", "Zada pre-model audit with real RAG context passes all checks", false, "preflight not sealed");
  ...
  record(
    ...
    candidateAuth.decision === PROFESSOR_V3_ZADA_PROSPECTIVE_SMOKE_DECISION_V1,
  );

  const audit = {
    version: "phase6a1-professor-v3-zada-executable-stack-sealing-audit-v1",
    generatedAt: new Date().toISOString(),
    decision: PROFESSOR_V3_ZADA_EXECUTABLE_STACK_FINALIZATION_DECISION_V1,
    candidateDecision: PROFESSOR_V3_ZADA_PROSPECTIVE_SMOKE_DECISION_V1,
    realZadaProspectiveModelSmoke: "NOT_YET_AUTHORIZED",
