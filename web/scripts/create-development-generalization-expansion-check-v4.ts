/**
 * Create sealed expansion-check-v4 holdout BEFORE v1.27 parser tuning.
 */
...
import {
  EXPANSION_CHECK_V4_COUNT,
  EXPANSION_CHECK_V4_SEEDS,
} from "./development-generalization-expansion-check-v4-seeds";
...
const REVIEWER = "expansion-check-v4-selector";
const CERTIFIER = "development-generalization-expansion-check-v4-builder";
...
async function main() {
  ...
  for (let i = 0; i < EXPANSION_CHECK_V4_SEEDS.length; i++) {
    const seed = EXPANSION_CHECK_V4_SEEDS[i];
    const id = `dev-exp-chk-v4-${String(i + 1).padStart(3, "0")}`;
    ...
    testCase = {
      ...
      evaluationSetVersion: "development-generalization-expansion-check-v4",
  ...
  assertNoOracleIdOverlap(cases, excluded, "development_generalization_expansion_check_v4");

  const stratumCounts = Object.fromEntries(
    [...new Set(EXPANSION_CHECK_V4_SEEDS.map((s) => s.stratum))].map((s) => [
      s,
      EXPANSION_CHECK_V4_SEEDS.filter((x) => x.stratum === s).length,
    ]),
  );

  const envelope: EvalDatasetEnvelope & { cases: CatalogEvalCase[]; checkHoldout: Record<string, unknown> } = {
    setClassification: "development_generalization_expansion_check_v4",
    evaluationSetVersion: "development-generalization-expansion-check-v4",
    taxonomyVersion: TAXONOMY_VERSION,
    contentHash: "",
    cases,
    ...
    checkHoldout: {
      ...
      frozenAt: reviewedAt,
      stratumCounts,
      caseCount: EXPANSION_CHECK_V4_COUNT,
      policy: "Sealed before v1.27 parser tuning; do not run v1.26 against this holdout",
    },
    goldCertification: {
  ...
  const outPath = resolve(process.cwd(), "data/oracle-action-eval-development-generalization-expansion-check-v4.json");
  writeFileSync(outPath, `${JSON.stringify(envelope, null, 2)}\n`, "utf8");
  mkdirSync(resolve(process.cwd(), "data/milestones/rc2-development-planning"), { recursive: true });
  writeFileSync(
    resolve(process.cwd(), "data/milestones/rc2-development-planning/expansion-check-v4-selection.json"),
  ...
  console.log(JSON.stringify({ outPath, caseCount: cases.length, contentHash: envelope.contentHash, stratumCounts }, null, 2));
}
