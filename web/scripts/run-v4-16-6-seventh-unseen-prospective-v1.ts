/**
 * PROFESSOR v4.16.6 — Seventh unseen Commander search-execution prospective.
 * Decision: PROFESSOR_V4_16_6_SEVENTH_UNSEEN_COMMANDER_SEARCH_EXECUTION_AND_RECOVERY_LIVE_V1_AUTHORIZED
 */
...
const OUT_DIR = ensureProspectiveOutputDirectory(
  resolve(process.cwd(), "data/milestones/deck-synthesis/v4-16-6-seventh-unseen-prospective"),
);
const RANDOM_SEED = Number(process.env.PROFESSOR_V4_16_6_SEED ?? "416607001");
...
async function runProspective() {
  ...
  for (let pass = 0; pass < 100; pass++) {
    ...
    for (const ex of telAfter?.executions ?? []) {
      if (!seenExecutionIds.has(ex.executionId)) {
        ...
        structuralExecutions.push({ pass: pass + 1, ...executionReport(ex) });
      }
    }
    ...
    if (countAfter > countBefore) {
      ...
      if (isNormalAssemblyPhase(phaseAfter)) {
        normalAssemblyExecutions.push({
          ...
          telemetryBuildPhase: telAfter?.buildPhase,
          councilBuildPhase: csAfter.buildPhase,
          libraryCount: countAfter,
        });
      }
    }
  ...
  const acceptanceMatrix: Record<string, AcceptanceVerdictV4165> = {
    ...
    NORMAL_ASSEMBLY: gate(normalAssemblyExecutions.length > 0 || fillRatio >= 0.5 || structuralComplete),
  ...
  const report = {
    decision: "PROFESSOR_V4_16_6_SEVENTH_UNSEEN_COMMANDER_SEARCH_EXECUTION_AND_RECOVERY_LIVE_V1_AUTHORIZED",
    primaryQuestion:
      "Can Professor execute structural searches with retriever evidence, recover from stalls phase-aware, and terminate only with execution-backed exhaustion truth?",
  ...
  writeFileSync(
    ...
    `# PROFESSOR v4.16.6 Seventh Unseen Prospective — Search Execution & Recovery
...
- **Executions:** ${executionsFinal.length} (searchExecutionSuccess=${searchExecutionSuccess})
...
See prospective-report-v1.json for phase transitions, structural executions, recompute safety, and build pass log.
`,
  );
