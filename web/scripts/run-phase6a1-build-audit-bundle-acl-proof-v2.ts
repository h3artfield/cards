#!/usr/bin/env npx tsx
/** Build ACL proof v2 audit bundle acl-proof-v2.zip */
...
const ZIP_PATH = resolve(OUT, "acl-proof-v2.zip");
const STAGING = resolve(OUT, ".audit-bundle-acl-proof-v2-staging");
const MANIFEST_NAME = "phase6a1-professor-plan-audit-bundle-acl-proof-v2-manifest.json";
const BYTE_PIN_NAME = "phase6a1-professor-plan-audit-bundle-acl-proof-v2-byte-pin.json";
...
async function main() {
  ...
  for (const f of [
    "phase6a1-professor-plan-separate-authority-checkout-acl-proof-v2.json",
    "phase6a1-professor-plan-separate-authority-checkout-acl-proof-spec-v2.json",
    "phase6a1-professor-plan-separate-authority-checkout-acl-proof-v1.json",
    "phase6a1-professor-plan-acl-proof-v1-independent-reaudit-gpt56sol-v1.json",
  ...
  for (const rel of [
    "web/scripts/lib/phase6a1-acl-implementation-container-probe-v1.mjs",
    "web/scripts/run-phase6a1-prove-separate-authority-checkout-acl-v2.ts",
    "web/scripts/run-phase6a1-build-audit-bundle-acl-proof-v2.ts",
    ".gitignore",
  ]) {
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        version: "phase6a1-professor-plan-audit-bundle-acl-proof-v2-manifest",
        generatedAt: GENERATED_AT,
        artifact: "acl-proof-v2.zip",
        zipEntryCount: staged.length + 1,
        purpose: "ACL proof v2 — Docker mount implementation execution boundary",
        supersedes: "acl-proof-v1.zip",
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        version: "phase6a1-professor-plan-audit-bundle-acl-proof-v2-byte-pin",
        generatedAt: GENERATED_AT,
        artifact: "acl-proof-v2.zip",
        byteSha256,
        manifestArtifact: MANIFEST_NAME,
        manifestSha256: sha256File(manifestPath),
        aclProofArtifact: "phase6a1-professor-plan-separate-authority-checkout-acl-proof-v2.json",
        aclProofSha256: sha256File(resolve(OUT, "phase6a1-professor-plan-separate-authority-checkout-acl-proof-v2.json")),
        uploadTogether: ["acl-proof-v2.zip", BYTE_PIN_NAME],
      },
      null,
      2,
    ),
