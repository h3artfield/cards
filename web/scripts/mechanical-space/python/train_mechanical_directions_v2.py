#!/usr/bin/env python3
"""
Mechanical Ontology Precision v2

Linear directions on frozen BGE. Hierarchy, hard negatives, polarity, audit.
Unknown is never treated as a trusted negative. No re-embed. No OpenAI.
"""
...
from mechanical_ontology_v2 import (
    BY_ID,
    COMPILED,
    CONCEPTS,
    is_token_card,
...
def main() -> None:
    ...
    for i in range(n):
        ...
        for hid in map_oracle_text(texts[oracle_ids[i]]):
            Y[i, cid[hid]] = 1
        if is_token_card(types[oracle_ids[i]], names[oracle_ids[i]]):
    ...
    (OUT / "ontology.json").write_text(
        json.dumps({"version": "mechanical-ontology-v2", "concepts": CONCEPTS, "hierarchyNote": "parent/child; kind=capability|dependency|audit"}, indent=2) + "\n",
        encoding="utf-8",
    )
    ...
    gpu = gpu_report()
    report = {
        "version": "mechanical-directions-v2",
    ...
    (MS / "mechanical-ontology-precision-v2-report.json").write_text(
        json.dumps(
            {
                k: report[k]
                for k in report
        ...
        json.dumps(
            {
                ...
                "preciseOutlets": next(({"id": r["id"], "class": r["directionClass"], "mAP": r["holdout"].get("mAP"), "hn": r["hardNegative"]} for r in per if r["id"] == "REPEATABLE_SACRIFICE_OUTLET"), None),
                "edict": next(({"mAP": r["holdout"].get("mAP"), "hn": r["hardNegative"], "class": r["directionClass"]} for r in per if r["id"] == "EDICT"), None),
                "counter": next(({"mAP": r["holdout"].get("mAP"), "hn": r["hardNegative"], "class": r["directionClass"]} for r in per if r["id"] == "COUNTER_SPELL"), None),
                "costRed": next(({"mAP": r["holdout"].get("mAP"), "hn": r["hardNegative"], "class": r["directionClass"]} for r in per if r["id"] == "COST_REDUCTION"), None),
            },
            indent=2,
        )
    )
