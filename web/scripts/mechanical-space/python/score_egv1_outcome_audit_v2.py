#!/usr/bin/env python3
"""
Outcome Audit v2 scoring. Loads predictions only after outcomes are frozen.
"""
...
def score_pair(above: set[tuple[str, str]], a: str, b: str) -> str | int:
    if (a, b) in above:
        return 1
    if (b, a) in above:
        return 0
...
def main() -> None:
    ...
    if load_json(V1 / "XO4_PERMANENT.json").get("doNotRevise") is not True:
        raise SystemExit("XO4 permanent freeze missing")
    ...
    raw = (OUT / "transcript-outcomes-v2.json").read_bytes()
    if hashlib.sha256(raw).hexdigest() != frozen["outcomesSha256"]:
        raise SystemExit("outcome artifact changed after freeze")
    outcomes = json.loads(raw.decode("utf-8"))
    ...
    report = {
        ...
        "transcriptsOpenedForMechanisms": False,
        "gameplaySummaryUsed": False,
    }
    (OUT / "pair-scores-v2.json").write_text(json.dumps({"n": len(scored), "pairs": scored}, indent=2) + "\n", encoding="utf-8")
    (OUT / "report.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    (OUT / "IMMUTABLE.json").write_text(
        json.dumps(
            {
                "artifactType": "ExternalGameplayOutcomeAudit",
                "version": "external-gameplay-outcome-audit-v2",
                "label": "TRANSCRIPT_ASSISTED",
                "status": "FROZEN",
                "call": call,
                "sourceFlag": source_flag,
        ...
        json.dumps(
            {
                ...
                "ci": ci,
                "kinds": dict(kinds),
                "retrieved": outcomes.get("nTranscriptsRetrieved"),
            },
            indent=2,
        )
    )
