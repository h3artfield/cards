#!/usr/bin/env npx tsx
/**
 * Pre-gold amendment v4: canonical ontology v2 + synergy graph v4 recompute only.
 */
...
import { PACKAGE_SYNERGY_GRAPH_V4_VERSION } from "./lib/phase6a1-package-synergy-graph-v4";
import { CANONICAL_RESOURCE_ONTOLOGY_V2_VERSION } from "./lib/phase6a1-canonical-resource-ontology-v2";
...
const SPENT_AMENDED_V3_DIR = resolve(
  "data/milestones/deck-synthesis/phase6a1-professor-plan-experiment-v3-amended-v3-spent-diagnostic",
);
const SOURCE_AMENDED_V3_DIR = resolve("data/milestones/deck-synthesis/phase6a1-professor-plan-experiment-v3-amended-v3");
const AMENDED_V4_DIR = resolve("data/milestones/deck-synthesis/phase6a1-professor-plan-experiment-v3-amended-v4");
const AMENDED_V4_CASES_DIR = resolve(AMENDED_V4_DIR, "cases");
...
function recomputeThreeLens(casePath: string): void {
  ...
  record.synergyGraphVersion = PACKAGE_SYNERGY_GRAPH_V4_VERSION;
  record.canonicalResourceOntologyVersion = CANONICAL_RESOURCE_ONTOLOGY_V2_VERSION;
  ...
  record.preGoldAmendment = {
    version: "phase6a1-professor-plan-experiment-v3-amended-pre-gold-v4",
...
async function main() {
  const skipArchive = process.env.PROFESSOR_V3_AMEND_V4_SKIP_ARCHIVE === "1";

  mkdirSync(AMENDED_V4_CASES_DIR, { recursive: true });

  if (!skipArchive) {
    mkdirSync(SPENT_AMENDED_V3_DIR, { recursive: true });
    for (const name of readdirSync(SOURCE_AMENDED_V3_DIR)) {
      const src = resolve(SOURCE_AMENDED_V3_DIR, name);
      const dest = resolve(SPENT_AMENDED_V3_DIR, name);
      if (name === "cases") {
  ...
  for (const file of readdirSync(resolve(SOURCE_AMENDED_V3_DIR, "cases")).filter((f) => f.endsWith(".json"))) {
    const dest = resolve(AMENDED_V4_CASES_DIR, file);
    if (!existsSync(dest)) {
      copyFileSync(resolve(SOURCE_AMENDED_V3_DIR, "cases", file), dest);
    }
  }

  for (const file of readdirSync(AMENDED_V4_CASES_DIR).filter((f) => f.endsWith(".json"))) {
    recomputeThreeLens(resolve(AMENDED_V4_CASES_DIR, file));
  }

  const caseFiles = readdirSync(AMENDED_V4_CASES_DIR)
    .filter((f) => f.endsWith(".json"))
    .sort();
  const casePaths = caseFiles.map((f) => resolve(AMENDED_V4_CASES_DIR, f));
  ...
  const manifest = {
    version: "phase6a1-professor-plan-experiment-manifest-v3-amended-v4",
    generatedAt: new Date().toISOString(),
    sourceSpentDiagnostic: "phase6a1-professor-plan-experiment-v3-amended-v3-spent-diagnostic",
    preGoldAuditReference: "phase6a1-professor-plan-experiment-v3-amended-v3-pre-gold-reaudit-gpt56sol-v1.json",
    amendment: {
      version: "phase6a1-professor-plan-experiment-v3-amended-pre-gold-v4",
      professorRerun: false,
      canonicalResourceOntologyVersion: CANONICAL_RESOURCE_ONTOLOGY_V2_VERSION,
      synergyGraphVersion: PACKAGE_SYNERGY_GRAPH_V4_VERSION,
      threeLensSelectorVersion: THREE_LENS_PORTFOLIO_SELECTOR_V5_VERSION,
    },
    authorization: {
      semanticRetrievalFreeze: "FROZEN",
      professorV3FormalRun: "AMENDED_PRE_GOLD_V4",
  ...
  const manifestPath = resolve(AMENDED_V4_DIR, "phase6a1-professor-plan-experiment-manifest-v3-amended-v4.json");
  ...
  const sealed = {
    version: "phase6a1-professor-plan-experiment-sealed-v3-amended-v4",
    sealedAt,
    caseCount: caseRecords.length,
    manifestSha256,
    caseSetSha256,
    sealKind: preGoldClear ? "AMENDED_V4_AWAIT_PRE_GOLD_REAUDIT" : "AMENDED_V4_INTERIM",
    preGoldAuditStatus: preGoldClear ? "AWAITING_INDEPENDENT_REAUDIT" : "BLOCKED_VALIDATION_OR_GRAPH_QA",
    goldEvaluationStatus: "NOT_RUN",
    note: "Amendment v4: qualifier-preserving canonical compatibility. Professor outputs unchanged.",
    provenanceSidecar: "phase6a1-professor-plan-experiment-v3-seal-provenance-sidecar-v1.json",
    predecessorAmendedPopulation: "phase6a1-professor-plan-experiment-v3-amended-v3-spent-diagnostic",
  };

  const report = {
    version: "phase6a1-professor-plan-experiment-report-v3-amended-v4",
    generatedAt: sealedAt,
    overallStatus: preGoldClear ? "AMENDED_V4_AWAIT_PRE_GOLD_REAUDIT" : "AMENDED_V4_PARTIAL_OR_BLOCKED",
    summary: manifest.population,
    goldComparison: "KEEP_SEALED",
  };

  writeFileSync(resolve(AMENDED_V4_DIR, "phase6a1-professor-plan-experiment-sealed-v3-amended-v4.json"), JSON.stringify(sealed, null, 2));
  writeFileSync(resolve(AMENDED_V4_DIR, "phase6a1-professor-plan-experiment-report-v3-amended-v4.json"), JSON.stringify(report, null, 2));

  writeFileSync(
    resolve(AMENDED_V4_DIR, "phase6a1-professor-plan-experiment-v3-amended-v4-completion-v1.json"),
    JSON.stringify(
      {
        version: "phase6a1-professor-plan-experiment-v3-amended-v4-completion-v1",
        completedAt: sealedAt,
        status: preGoldClear ? "AMENDED_V4_AWAIT_PRE_GOLD_REAUDIT" : "AMENDED_V4_PARTIAL_OR_BLOCKED",
        predecessor: {
          version: "phase6a1-professor-plan-experiment-v3-amended-v3-completion-v1",
          status: "SPENT_DIAGNOSTIC_PRE_GOLD_BLOCK",
          archive: "phase6a1-professor-plan-experiment-v3-amended-v3-spent-diagnostic",
          preGoldReaudit: "phase6a1-professor-plan-experiment-v3-amended-v3-pre-gold-reaudit-gpt56sol-v1.json",
          preGoldReauditDisposition: "BLOCK",
        },
        repairs: {
          canonicalResourceOntologyVersion: CANONICAL_RESOURCE_ONTOLOGY_V2_VERSION,
          synergyGraphVersion: PACKAGE_SYNERGY_GRAPH_V4_VERSION,
          qualifierPreservingCompatibility: true,
          payoffNonCausal: true,
          semanticRegressionAssertions: 6,
          professorRerun: false,
        },
        population: manifest.population,
        artifacts: {
          manifest: "phase6a1-professor-plan-experiment-manifest-v3-amended-v4.json",
          manifestSha256,
          sealed: "phase6a1-professor-plan-experiment-sealed-v3-amended-v4.json",
          caseSetSha256,
          report: "phase6a1-professor-plan-experiment-report-v3-amended-v4.json",
        },
        gateDisposition: {
          semanticRetrievalFreeze: "FROZEN",
          professorV3FormalRun: "AMENDED_PRE_GOLD_V4",
  ...
  writeFileSync(
    resolve("data/milestones/deck-synthesis/phase6a1-professor-plan-experiment-v3-amended-v3-spent-diagnostic-manifest-v1.json"),
    JSON.stringify(
      {
        version: "phase6a1-professor-plan-experiment-v3-amended-v3-spent-diagnostic-manifest-v1",
        spentAt: sealedAt,
        status: "SPENT_DIAGNOSTIC_PRE_GOLD_BLOCK",
        policy: "Amended v3 population preserved after re-audit BLOCK on canonical resource compatibility. Do not use for gold comparison.",
        preGoldReaudit: "phase6a1-professor-plan-experiment-v3-amended-v3-pre-gold-reaudit-gpt56sol-v1.json",
        successor: "phase6a1-professor-plan-experiment-v3-amended-v4",
      },
      null,
      2,
    ),
  ...
  copyFileSync(
    "C:/Users/h3art/Downloads/phase6a1-professor-plan-experiment-v3-amended-v3-pre-gold-reaudit-gpt56sol-v1.json",
    resolve("data/milestones/deck-synthesis/phase6a1-professor-plan-experiment-v3-amended-v3-pre-gold-reaudit-gpt56sol-v1.json"),
  );

  console.log(
    JSON.stringify(
      {
        amendedV4Dir: AMENDED_V4_DIR,
        manifestSha256,
        caseSetSha256,
        population: manifest.population,
        preGoldClear,
