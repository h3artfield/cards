...
export function inferSupportedPrimitiveFromEvidence(
  ...
  for (const primitive of PRIMITIVE_ACTION_TYPES) {
    const patterns: Record<PrimitiveActionType, RegExp> = {
      ...
      put_onto_battlefield: /\b(?:put (?:that |it(?:self)? )(?:card )?onto the battlefield|puts? all [\w ]+ onto the battlefield|put [\w ]+ from (?:your |a |their )?(?:hand|graveyard|exile)[\w ]* onto the battlefield)\b/i,
