#!/usr/bin/env npx tsx
/**
 * Archetype Discovery v1.3.2 — Phase 5.3.2 QA + regression manifest.
 * Deterministic QA uses structuralAcceptanceProxy — NOT human gold accuracy.
 */
...
function autoAdjudicateMechanical(r: CaseResult): {
  ...
  if (primary.driverPropagationFailure || (primary.drivers.length === 0 && primary.payoffs.length > 0 && depth.incidentalSupport > 0.25)) {
    return {
...
async function main() {
  ...
  const structuralAccepted = mechanicalAdjudications.filter((a) => a.outcome === "ACCEPTED").length;
  const structuralAcceptanceProxy =
    mechanicalAdjudications.length > 0 ? structuralAccepted / mechanicalAdjudications.length : 0;
  ...
  const devGate = {
    structuralAcceptanceProxyMin: 0.9,
    ...
    mechanicalDirectionPrecisionProxy: structuralAcceptanceProxy,
    ...
    note: "structuralAcceptanceProxy is automated regression only — Phase 5 gate uses human adjudication.",
    pass:
      structuralAcceptanceProxy >= 0.9 &&
  ...
  const qaReport = {
    version: "archetype-discovery-v1.3.2-phase5.3.2-qa",
    ...
    metrics: {
      primarySynthesisGate: {
        ...
        note: "Human mechanical review is the Phase 5 acceptance gate; this proxy is for regression only.",
      },
    ...
    authorization: {
      ...
      blindV2Creation: "WAIT_UNTIL_HUMAN_REVIEW_PASS_AND_FREEZE",
      phase6: "WAIT",
      optimizer: "WAIT",
  ...
  const reviewPath = resolve(outDir, "archetype-discovery-v1.3.2-mechanical-review-package.json");
  const freezePath = resolve(outDir, "archetype-discovery-v1.3.2-freeze-manifest.json");
