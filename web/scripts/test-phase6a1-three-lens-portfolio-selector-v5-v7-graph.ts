...
const amendedV6Dir = resolve("data/milestones/deck-synthesis/phase6a1-professor-plan-experiment-v3-amended-v6/cases");

function loadCasePackages(caseId: string): SemanticPackage[] {
  const record = JSON.parse(readFileSync(resolve(amendedV6Dir, `${caseId}.json`), "utf8")) as {
...
for (const file of readdirSync(amendedV6Dir).filter((f) => f.endsWith(".json"))) {
