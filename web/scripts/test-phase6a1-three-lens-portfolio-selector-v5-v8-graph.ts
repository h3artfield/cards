...
const amendedV7Dir = resolve("data/milestones/deck-synthesis/phase6a1-professor-plan-experiment-v3-amended-v7/cases");

function loadCasePackages(caseId: string): SemanticPackage[] {
  const record = JSON.parse(readFileSync(resolve(amendedV7Dir, `${caseId}.json`), "utf8")) as {
...
assert(
  "hua tuo top library recovered must match",
...
assert(
  "hua tuo accessible recovered must not match top library",
  !causalEdges(huaTuo).some(
    (e) =>
...
const erinis = selectThreeLensPortfoliosV5("blindv5-11-commander-background", loadCasePackages("blindv5-11-commander-background"));
assert(
  "erinis recovered mana not compound mana and interaction",
  !causalEdges(erinis).some(
    (e) =>
...
assert(
  "erinis recovered mana may match mana development atom",
  causalEdges(erinis).some(
    (e) =>
      /recovered mana development/i.test(e.support.producerText) &&
...
assert(
  "toolbox singular power4 not multiple power4",
...
for (const file of readdirSync(amendedV7Dir).filter((f) => f.endsWith(".json"))) {
