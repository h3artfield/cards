#!/usr/bin/env npx tsx
/**
 * Phase 5.4.1 — Full 118-case human mechanical + retrieval review.
...
async function main() {
  ...
  const report = {
    version: "archetype-discovery-v1.4.1-human-mechanical-review",
    ...
    authorization: {
      ...
      phase541: "COMPLETE — awaiting gate",
      phase54Freeze: "WAIT",
      blindV3: "WAIT",
      phase6: "WAIT",
  ...
  const outPath = resolve(outDir, "archetype-discovery-v1.4.1-human-mechanical-review.json");
  const matrixPath = resolve(outDir, "archetype-discovery-v1.4.0-to-v1.4.1-regression-matrix.json");
  writeFileSync(outPath, JSON.stringify(report, null, 2));
  writeFileSync(matrixPath, JSON.stringify({ version: "v1.4.0-to-v1.4.1", matrix: report.regressionMatrix, counts: regressionCounts, devGate541: report.devGate541 }, null, 2));

  console.log(JSON.stringify({ metrics: report.metrics, devGate541: report.devGate541, diagnosis: report.diagnosis, outPath, hash: createHash("sha256").update(JSON.stringify(report)).digest("hex") }, null, 2));
}
