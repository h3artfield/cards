/**
 * PROFESSOR v4.16.6.1 — Eighth unseen Commander authoritative build + structural recovery prospective.
 * Decision: PROFESSOR_V4_16_6_1_EIGHTH_UNSEEN_COMMANDER_AUTHORITATIVE_BUILD_AND_STRUCTURAL_RECOVERY_LIVE_V1_AUTHORIZED
 */
...
const OUT_DIR = ensureProspectiveOutputDirectory(
  resolve(process.cwd(), "data/milestones/deck-synthesis/v4-16-6-1-eighth-unseen-prospective"),
);
const RANDOM_SEED = Number(process.env.PROFESSOR_V4_16_6_1_SEED ?? "416608001");
...
async function runProspective() {
  ...
  for (let pass = 0; pass < 100; pass++) {
    ...
    for (const ex of telAfter?.executions ?? []) {
      if (!seenExecutionIds.has(ex.executionId)) {
        ...
        const exReport = { pass: pass + 1, ...executionReport(ex), rejectedSample: ex.rejected.slice(0, 5) };
        structuralExecutions.push(exReport);
        if (stallEpisode) {
          stallEpisode.executions = [...(stallEpisode.executions ?? []), executionReport(ex)];
        }
      }
    }
  ...
  const acceptanceMatrix: Record<string, AcceptanceVerdictV4165> = {
    ...
    "AUTHORITATIVE BUILD CONTROL": gate(authoritativeGateLive, !authoritativeGateAtCreativeComplete.shouldContinueAutoBuild),
    "FIRST ADVANCE": gate(firstAdvancePass),
    BUILD_PHASE_TRUTH: gate(buildPhaseTruth),
    ...
    "RETRIEVER EXECUTION": gate(
      executionsFinal.length > 0 && hasRetrieverExecution,
      executionsFinal.length === 0,
      executionsFinal.length === 0,
    ),
    GENUINE_SEARCH_ESCALATION: gate(
      genuineEscalation || structuralComplete || completeDeck,
      !genuineEscalation && executionsFinal.length >= 2,
      executionsFinal.length === 0,
    ),
    "CANDIDATE SUPPLY RECONCILIATION": gate(supplyReconciled || structuralComplete || exhaustionTerminal),
    "REJECTION EVIDENCE": gate(hasRejectionEvidence || executionsFinal.length === 0 || structuralComplete),
    "REJECTION-DRIVEN REFORMULATION": gate(rejectionReformulation || structuralComplete || completeDeck),
    "NO-OP TRUTH": gate(!noOpFailure || exhaustionTerminal || structuralComplete),
  ...
  const report = {
    decision: "PROFESSOR_V4_16_6_1_EIGHTH_UNSEEN_COMMANDER_AUTHORITATIVE_BUILD_AND_STRUCTURAL_RECOVERY_LIVE_V1_AUTHORIZED",
    primaryQuestion:
      "Can normal assembly build the deck, and if construction genuinely stalls, can Structural Research execute real searches, improve the deck, and return control to normal assembly?",
  ...
  writeFileSync(
    ...
    `# PROFESSOR v4.16.6.1 Eighth Unseen Prospective — Authoritative Build & Recovery
...
- **Legacy conflict at creative:** ${authoritativeGateAtCreativeComplete.legacyConflict}
