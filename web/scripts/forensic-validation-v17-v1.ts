/**
 * v17 forensic diagnosis v1 — 33 FN / 7 FP adjudication from preserved RC7 first run.
 * Run: cd web && npx tsx scripts/forensic-validation-v17-v1.ts
 */
...
const AGGREGATE_PATH = `${OUT_DIR}/validation-v17-rc7-execution-1-aggregate.json`;
const RAW_PATH = `${OUT_DIR}/validation-v17-rc7-execution-1-raw.json`;
const CERT_PATH = `${OUT_DIR}/validation-v17-gold-policy-certificate-v3.json`;
const CORR_PATH = `${OUT_DIR}/validation-v17-gold-policy-correction-report-v1.json`;
const FREEZE_PATH = `${OUT_DIR}/validation-v17-freeze-manifest-v152.json`;
const V16_ROOT_CAUSE_PATH = "data/milestones/validation-v16-certification/validation-v16-root-cause-ledger-v1.json";
...
function mapRc8PrimaryMechanism(row: Record<string, unknown>): string {
...
function buildRc8RootCauseLedger(
  fnLedger: Array<Record<string, unknown>>,
  fpLedger: Array<Record<string, unknown>>,
  v16Ledger: Array<{ proposedStructuralFamily?: string; caseId?: string }>,
): Record<string, unknown> {
  const genuine = [
    ...fnLedger.filter((r) => r.finalVerdict === "genuine_parser_fn"),
    ...fpLedger.filter((r) => r.finalVerdict === "genuine_parser_fp"),
  ];
  const v16Families = new Set(v16Ledger.map((r) => r.proposedStructuralFamily).filter(Boolean));
  const rows = genuine.map((row) => {
    const mechanism = mapRc8PrimaryMechanism(row);
    const v16Match = v16Ledger.some(
      (v) =>
        v.proposedStructuralFamily === mechanism ||
        (v.caseId && String(row.caseId).replace("vh17", "vh16") === v.caseId),
    );
    return {
      caseId: row.caseId,
      ...
      recurrence: {
        observedV15: false,
        observedV16: v16Match || v16Families.has(mechanism),
        observedV17: true,
      },
  ...
  return {
    ...
    version: "validation-v17-root-cause-ledger-v1",
    integrity: {
...
function v15GrammarCrossReference(fnLedger: Array<Record<string, unknown>>) {
  return V15_GRAMMAR_GAPS.map((gap) => {
    ...
    return {
      ...gap,
      v17IndependentRecurrence: hits.length > 0,
      v17Cases: hits.map((r) => r.caseId),
      note:
        hits.length > 0
          ? "Fresh v17 independently exposes same construction — stronger RC8 justification."
          : "No independent v17 recurrence — leave spent-v15 one-off alone.",
    };
  });
}
...
function adjudicateFn(
  ...
  gold: ExpectedPrimitiveAction,
  parse: OracleSemanticParse,
): { finalVerdict: FnVerdict; policyClass: string; reason: string; rc8FamilyHint?: string } {
  ...
  if (gold.actionType === "return_to_battlefield" && /to your hand/i.test(ev)) {
    return {
      ...
      rc8FamilyHint: "saga_planeswalker_semantics",
    };
  }
  ...
  if (gold.actionType === "play" && /play lands?/i.test(ev)) {
    return {
      ...
      rc8FamilyHint: "library_movement_distinction",
    };
  }
  ...
  if (gold.actionType === "play" && /You may play that card/i.test(ev)) {
    return {
      ...
      rc8FamilyHint: "granted_nested_generalization",
    };
  }
  ...
  if (gold.actionType === "cast") {
    if (/you may cast[^.]*without paying its mana cost/i.test(ev)) {
      return {
        ...
        rc8FamilyHint: "immediate_cast_grammar",
      };
    }
    if (/cast (?:instant and sorcery|creature spells from|spells with the chosen name|if you control)/i.test(ev)) {
      return {
        ...
        rc8FamilyHint: stratum.includes("immediate_cast")
    ...
    if (/cast spells or activate abilities that aren't mana abilities/i.test(ev)) {
      return {
        ...
        rc8FamilyHint: "replacement_effect_parsing",
      };
    }
    if (/cast an Ally spell/i.test(ev) && /Whenever you cast/i.test(oracle)) {
      return {
        ...
        rc8FamilyHint: "triggered_compound_grammar",
      };
    }
  ...
  if (gold.actionType === "sacrifice" || gold.actionType === "discard") {
    ...
    if (row.verdict === "valid_layer2_gold") {
      return {
        ...
        rc8FamilyHint: "player_subject_imperative_extraction",
      };
    }
  ...
  if (gold.actionType === "add_mana" && /Add \{/.test(ev)) {
    return {
      ...
      rc8FamilyHint: stratum.includes("granted") ? "granted_nested_generalization" : undefined,
    };
  }
  if (gold.actionType === "draw" && /draw/i.test(ev)) {
    ...
    if (emissions.length > 0) {
      return {
        ...
        rc8FamilyHint: "mdfc_face_provenance",
      };
    }
    return {
      finalVerdict: "genuine_parser_fn",
  ...
  if (gold.actionType === "return_to_hand" && /return target/i.test(ev)) {
    return {
      ...
      rc8FamilyHint: "zone_transition_effects",
    };
  }
  ...
  return {
    ...
    rc8FamilyHint: stratum.includes("granted")
      ? "granted_nested_generalization"
...
function adjudicateFp(
  ...
  if (observedAction === "cast" && /You may cast this /i.test(observedEvidence)) {
    return {
      ...
      rc8FamilyHint: "immediate_cast_grammar",
    } as ReturnType<typeof adjudicateFp> & { rc8FamilyHint?: string };
  }
...
function main() {
  ...
  if (envelope.parserExecutionCount !== 1) throw new Error("v17 must be spent (parserExecutionCount=1)");
  ...
  const fnLedger = allFn.map((row) => {
    ...
    return {
      ...
      rc8FamilyHint: adj.rc8FamilyHint ?? inferRc8FamilyHint({ ...row, finalVerdict: adj.finalVerdict, semanticOwner: semantic.semanticOwner, executionContext: semantic.executionContext }, "fn") ?? null,
  ...
  const fpLedger = allFp.map((row) => {
    ...
    return {
      ...
      policyClass: adj.policyClass,
      adjudicationReason: adj.reason,
      rc8FamilyHint: inferRc8FamilyHint({ ...row, finalVerdict: adj.finalVerdict }, "fp") ?? null,
  ...
  if (fpLedger.length !== 7 || fpIds.size !== 7) throw new Error("FP ledger integrity failed");
  ...
  for (const r of genuineParserFn) {
    const hint = (r.rc8FamilyHint as string) ?? `${r.coverageStratum}|${r.expectedAction}`;
    familyCensus[hint] = (familyCensus[hint] ?? 0) + 1;
  }
  for (const r of genuineParserFp) {
    const hint = (r.rc8FamilyHint as string) ?? `${r.coverageStratum}|${r.observedAction}`;
  ...
  const proposedRc8Families = Object.entries(familyCensus)
    ...
    .map(([family, count]) => ({
      ...
      auditCases: genuineParserFn.filter((r) => (r.rc8FamilyHint as string) === family).map((r) => r.caseId),
    }))
  ...
  const v16RootCause = JSON.parse(readFileSync(V16_ROOT_CAUSE_PATH, "utf8")) as {
    ledger: Array<{ proposedStructuralFamily?: string; caseId?: string }>;
  };
  const rc8RootCauseLedger = buildRc8RootCauseLedger(fnLedger, fpLedger, v16RootCause.ledger ?? []);
  ...
  const grantedV16MechanismRecurrence = grantedNestedSemanticAudit
    ...
    .map((row) => {
      const mechanism = mapRc8PrimaryMechanism({ ...row, coverageStratum: "challenge_granted_nested_actions" });
      return {
        caseId: row.caseId,
        mechanism,
  ...
  const artifact = {
    ...
    version: "validation-v17-forensic-adjudication-v1",
    frozen: true,
    frozenAt: new Date().toISOString(),
    policyStackPinned,
    officialV17: OFFICIAL,
    ...
    denominatorReconciliation: {
      prePolicyL2: freeze.validationV17?.prePolicyLayer2GoldPrimitiveCount ?? corr.prePolicyL2,
      certifiedL2: freeze.validationV17?.layer2GoldPrimitiveCount ?? corr.certifiedL2,
    ...
    policyBucketAudit: {
      ...
      rows: policyBucketAudit,
    },
    policyCorrectedDiagnostic: {
      label: "validation_v17_mismatch_adjudicated_forensic_v1",
      note: "NOT official v17 — forensic view only; official remains FAIL/SPENT",
      tp: diagTp,
      fp: diagFp,
      fn: diagFn,
  ...
  const outPath = resolve(OUT_DIR, "validation-v17-forensic-adjudication-v1.json");
  writeFileSync(outPath, `${JSON.stringify(artifact, null, 2)}\n`, "utf8");

  writeFileSync(
    resolve(OUT_DIR, "validation-v17-root-cause-ledger-v1.json"),
    `${JSON.stringify(rc8RootCauseLedger, null, 2)}\n`,
  );

  writeFileSync(
    resolve(OUT_DIR, "validation-v17-policy-adjudication-v1.json"),
    `${JSON.stringify(
      {
        artifactType: "ValidationPolicyAdjudication",
        version: "validation-v17-policy-adjudication-v1",
        parserBlind: true,
        ...
        policyBucketAudit: artifact.policyBucketAudit,
        genuineParserFailureCensus: artifact.genuineParserFailureCensus,
        proposedRc8Families: artifact.proposedRc8Families,
      },
      null,
      2,
    )}\n`,
  ...
  writeFileSync(
    resolve(OUT_DIR, "validation-v17-policy-corrected-diagnostic-v1.json"),
    `${JSON.stringify(
      {
        artifactType: "ValidationPolicyCorrectedDiagnostic",
        version: "validation-v17-policy-corrected-diagnostic-v1",
        officialSpentFirstRun: OFFICIAL,
        policyCorrected: artifact.policyCorrectedDiagnostic,
        adjudicationRef: "validation-v17-policy-adjudication-v1.json",
      },
      null,
      2,
    )}\n`,
  ...
  writeFileSync(
    resolve(OUT_DIR, "validation-v17-forensic-freeze-manifest-v1.json"),
    `${JSON.stringify(
      {
        manifestVersion: "validation-v17-forensic-freeze-v1",
        frozenAt: artifact.frozenAt,
        forensicArtifactPath: "validation-v17-forensic-adjudication-v1.json",
        forensicArtifactHash: createHash("sha256").update(JSON.stringify(artifact, null, 2)).digest("hex"),
        officialV17: OFFICIAL,
        note: "Frozen forensic ledger — official v16 result remains FAIL/SPENT.",
      },
      null,
      2,
    )}\n`,
