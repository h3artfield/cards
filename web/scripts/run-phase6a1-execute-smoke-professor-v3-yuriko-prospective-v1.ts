#!/usr/bin/env npx tsx
/** Yuriko prospective Professor v3 smoke runner v1 — live NO_MODEL auth; full executable stack when separately authorized. */
...
import { runProfessorV3YurikoProspectiveSmokeExecutionV1 } from "./lib/phase6a1-professor-v3-yuriko-prospective-smoke-execution-v1";

loadProjectEnvLocal();

async function main() {
  const result = await runProfessorV3YurikoProspectiveSmokeExecutionV1({
  ...
  console.log(
    JSON.stringify(
      {
        version: "phase6a1-professor-v3-smoke-yuriko-prospective-executed-v1",
