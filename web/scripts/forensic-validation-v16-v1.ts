/**
 * v16 forensic diagnosis v1 — 39 FN / 3 FP adjudication from preserved RC6 first run.
 * Run: cd web && npx tsx scripts/forensic-validation-v16-v1.ts
 */
...
const AGGREGATE_PATH = `${OUT_DIR}/validation-v16-rc6-execution-1-aggregate.json`;
const RAW_PATH = `${OUT_DIR}/validation-v16-rc6-execution-1-raw.json`;
const CERT_PATH = `${OUT_DIR}/validation-v16-gold-policy-certificate-v4.json`;
const CORR_PATH = `${OUT_DIR}/validation-v16-gold-policy-correction-report-v3.json`;
const FREEZE_PATH = `${OUT_DIR}/validation-v16-freeze-manifest-v150.json`;
...
function v15GrammarCrossReference(fnLedger: Array<Record<string, unknown>>) {
  return V15_GRAMMAR_GAPS.map((gap) => {
    ...
    return {
      ...gap,
      v16IndependentRecurrence: hits.length > 0,
      v16Cases: hits.map((r) => r.caseId),
      note:
        hits.length > 0
          ? "Fresh v16 independently exposes same construction — stronger RC7 justification."
          : "No independent v16 recurrence — leave spent-v15 one-off alone.",
    };
  });
}
...
function main() {
  ...
  if (envelope.parserExecutionCount !== 1) throw new Error("v16 must be spent (parserExecutionCount=1)");
  ...
  const fnLedger = allFn.map((row) => {
    ...
    return {
      ...
      rc7FamilyHint: adj.rc6FamilyHint ?? inferRc7FamilyHint({ ...row, finalVerdict: adj.finalVerdict, semanticOwner: semantic.semanticOwner, executionContext: semantic.executionContext }, "fn") ?? null,
  ...
  const fpLedger = allFp.map((row) => {
    ...
    return {
      ...
      policyClass: adj.policyClass,
      adjudicationReason: adj.reason,
      rc7FamilyHint: inferRc7FamilyHint({ ...row, finalVerdict: adj.finalVerdict }, "fp") ?? null,
  ...
  if (fnLedger.length !== 39 || fnIds.size !== 39) throw new Error("FN ledger integrity failed");
  if (fpLedger.length !== 3 || fpIds.size !== 3) throw new Error("FP ledger integrity failed");
  ...
  for (const r of genuineParserFn) {
    const hint = (r.rc7FamilyHint as string) ?? `${r.coverageStratum}|${r.expectedAction}`;
    familyCensus[hint] = (familyCensus[hint] ?? 0) + 1;
  }
  for (const r of genuineParserFp) {
    const hint = (r.rc7FamilyHint as string) ?? `${r.coverageStratum}|${r.observedAction}`;
      ...
      adjudicationNote:
        semantics.ownerBucket === "granted_object" && semantics.contextFamily === "granted_ability"
          ? row.finalVerdict === "genuine_parser_fn"
            ? "Likely real granted-rule extraction miss — RC7 structural family candidate."
      ...
      adjudicationNote:
        semantics.replacementLayer === "L1_intercepted_event"
          ...
          : semantics.replacementLayer === "L2_replacement_consequence" && row.finalVerdict === "genuine_parser_fn"
            ? "Legitimate replacement consequence miss — RC7 replacement family candidate."
            : row.finalVerdict === "invalid_gold"
              ? "Gold labels non-consequence under replacement policy."
              : "Parser-blind semantic adjudication required before RC7 replacement work.",
    };
  });
  ...
  const proposedRc7Families = Object.entries(familyCensus)
    .filter(([k]) => !k.startsWith("FP:"))
    .map(([family, count]) => ({
      family: family.replace(/^FP:/, ""),
      genuineParserFn: count,
      genuineParserFp: familyCensus[`FP:${family.replace(/^FP:/, "")}`] ?? 0,
      status: "proposed_wait_rc7_authorization",
      auditCases: genuineParserFn.filter((r) => (r.rc7FamilyHint as string) === family).map((r) => r.caseId),
    }))
  ...
  const artifact = {
    ...
    version: "validation-v16-forensic-adjudication-v1",
    frozen: true,
    frozenAt: new Date().toISOString(),
    policyStackPinned,
    officialV16: OFFICIAL,
    ...
    denominatorReconciliation: {
      prePolicyL2: freeze.validationV16?.prePolicyLayer2GoldPrimitiveCount ?? corr.layer2GoldBefore,
      certifiedL2: freeze.validationV16?.layer2GoldPrimitiveCount ?? corr.layer2GoldAfter,
    ...
    replacementSemanticAudit: {
      stratumRecallOfficial: 0.6923076923076923,
    ...
    policyCorrectedDiagnostic: {
      label: "validation_v16_mismatch_adjudicated_forensic_v1",
      note: "NOT official v16 — forensic view only; official remains FAIL/SPENT",
      tp: diagTp,
      fp: diagFp,
      fn: diagFn,
  ...
  const outPath = resolve(OUT_DIR, "validation-v16-forensic-adjudication-v1.json");
  writeFileSync(outPath, `${JSON.stringify(artifact, null, 2)}\n`, "utf8");

  writeFileSync(
    resolve(OUT_DIR, "validation-v16-policy-adjudication-v1.json"),
    `${JSON.stringify(
      {
        artifactType: "ValidationPolicyAdjudication",
        version: "validation-v16-policy-adjudication-v1",
        parserBlind: true,
        ...
        proposedRc7Families: artifact.proposedRc7Families,
      },
      null,
      2,
    )}\n`,
  ...
  writeFileSync(
    resolve(OUT_DIR, "validation-v16-policy-corrected-diagnostic-v1.json"),
    `${JSON.stringify(
      {
        artifactType: "ValidationPolicyCorrectedDiagnostic",
        version: "validation-v16-policy-corrected-diagnostic-v1",
        officialSpentFirstRun: OFFICIAL,
        policyCorrected: artifact.policyCorrectedDiagnostic,
        adjudicationRef: "validation-v16-policy-adjudication-v1.json",
      },
      null,
      2,
    )}\n`,
  ...
  writeFileSync(
    resolve(OUT_DIR, "validation-v16-forensic-freeze-manifest-v1.json"),
    `${JSON.stringify(
      {
        manifestVersion: "validation-v16-forensic-freeze-v1",
        frozenAt: artifact.frozenAt,
        forensicArtifactPath: "validation-v16-forensic-adjudication-v1.json",
