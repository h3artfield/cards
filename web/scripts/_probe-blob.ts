...
const hashes = paths.map((p) => createHash("sha256").update(readFileSync(resolve(p))).digest("hex"));
console.log(JSON.stringify({ blob: createHash("sha256").update(hashes.join("\n")).digest("hex"), paths: paths.length }));
