/**
 * Shared fail-closed catalog-data-state verification core (commitment v2).
 */
...
import { loadDeckResolutionCatalog } from "./load-deck-resolution-catalog";
import { combinedGoldenOracleText, goldenOracleTextHash, loadGoldenCatalogIndex } from "./load-golden-catalog-index";
...
export type CatalogVerificationCoreResult = {
...
export async function runCatalogDataStateVerificationCore(input: {
  repoRoot: string;
  commitmentPath: string;
  verifyPilotCommanders?: boolean;
}): Promise<CatalogVerificationCoreResult> {
  const { repoRoot, commitmentPath } = input;
...
export function buildCatalogVerificationArtifact(input: {
  phase: CatalogVerificationPhase;
  version: string;
  verifierScript: string;
  verifiedAt: string;
  core: CatalogVerificationCoreResult;
