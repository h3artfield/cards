/**
 * Package synergy graph v7 — v6 symmetric layer frozen; v5 destination/zone compatibility for RECOVERED_CREATURE.
 */
import type { SemanticPackage } from "../../src/lib/deck-synthesis/professor-planning-contracts-v2";
import {
  CANONICAL_RESOURCE_ONTOLOGY_V5_VERSION,
...
} from "./phase6a1-canonical-resource-ontology-v5";

export const PACKAGE_SYNERGY_GRAPH_V7_VERSION = "phase6a1-package-synergy-graph-v7";
...
export type PackageSynergyGraphV7 = {
...
const ALLOWED_V6_TO_V7_CAUSAL_REMOVALS = new Set([
  causalEdgeRecordKey({
    fromPackageId: "package-immediate-topdeck-conversion",
    toPackageId: "package-redundant-recovery-contingency",
    kind: "REQUIRES_FROM",
    reason: "",
    resource: "RECOVERED_CREATURE",
    directed: true,
    support: {
      producerField: "producedResources",
      producerIndex: 0,
      producerText: "Recovered creatures in accessible zones",
      consumerField: "requiredResources",
      consumerIndex: 0,
      consumerText: "A creature card placed on top of the library",
    },
  }),
  causalEdgeRecordKey({
    fromPackageId: "package-redundant-recovery-contingency",
    toPackageId: "package-immediate-topdeck-conversion",
    kind: "PRODUCES_FOR",
    reason: "",
    resource: "RECOVERED_CREATURE",
    directed: true,
...
export function auditCausalPredecessorDiff(
  ...
  const unexpectedRemoved = removed.filter((key) => !ALLOWED_V6_TO_V7_CAUSAL_REMOVALS.has(key));
  const unexpectedAdded = added;
  const passed =
    unexpectedRemoved.length === 0 &&
    unexpectedAdded.length === 0 &&
    removed.length === ALLOWED_V6_TO_V7_CAUSAL_REMOVALS.size;
  return {
    passed,
...
export function buildPackageSynergyGraphV7(caseId: string, packages: SemanticPackage[]): PackageSynergyGraphV7 {
  ...
  const qa: SynergyGraphQA = {
    version: PACKAGE_SYNERGY_GRAPH_V7_VERSION,
    ontologyVersion: CANONICAL_RESOURCE_ONTOLOGY_V5_VERSION,
...
export function enrichPackagesWithSynergyGraphV7(
  caseId: string,
  packages: SemanticPackage[],
): {
  packages: SemanticPackage[];
  ...
  const graph = buildPackageSynergyGraphV7(caseId, clean);
