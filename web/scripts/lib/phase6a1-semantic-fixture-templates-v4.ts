...
export function buildTokenSwarmFuelGap(caseId: string, commanders: string[], cz: CaseSemanticFixture["commandZoneConfiguration"]): CaseSemanticFixture {
  ...
  base.lenses.DEPENDENT_SYNERGY.resourceGraph.edges = base.lenses.DEPENDENT_SYNERGY.resourceGraph.edges.filter(
    (e) => e.fromNodeId !== "dep-fuel" && e.toNodeId !== "dep-fuel" && e.fromNodeId !== "sacrifice fodder",
  );
  const engine = base.lenses.DEPENDENT_SYNERGY.validatedPackages.find((p) => p.packageRefId === "dep-engine");
  if (engine) {
    engine.producedResources = ["mana"];
    engine.requiredResources = [];
  }
  return base;
}
...
export function buildGraveyardRecoveryGap(caseId: string, commanders: string[], cz: CaseSemanticFixture["commandZoneConfiguration"]): CaseSemanticFixture {
  const dep = {
    ...
    validatedPackages: [
      pkg("dep-engine", "Self-mill engine", ["graveyard creatures"], ["self-mill"], [], { resourceTransformations: ["self-mill feed"] }),
...
export function buildProtectionGap(caseId: string, commanders: string[], cz: CaseSemanticFixture["commandZoneConfiguration"]): CaseSemanticFixture {
  const dep = {
    ...
    validatedPackages: [
      pkg("dep-engine", "Attrition engine", ["persistent engine"], ["mana"], [], { semanticRequirements: ["interaction pressure against removal"] }),
...
export function buildFinisherGap(caseId: string, commanders: string[], cz: CaseSemanticFixture["commandZoneConfiguration"]): CaseSemanticFixture {
  const dep = {
    ...
    validatedPackages: [
      ...
      pkg("dep-payoff", "Incremental combat value", ["combat damage"], ["evasive attackers"], ["incremental value"]),
    ],
    rejectedPackages: [],
    resourceGraph: {
...
/** DEV36 opaque scenario codes — internal scenario kinds are not exported. */
export const DEV36_SCENARIO_CODES = Array.from({ length: 36 }, (_, i) => `D${String(i + 1).padStart(2, "0")}`) as readonly string[];

const DEV36_INTERNAL_SCENARIOS: ScenarioKind[] = [
  "token_closed",
  "token_fuel_gap",
  "partner_maint_closed",
  "partner_maint_gap",
...
export function buildFixtureForDevScenarioCode(
  code: string,
  caseId: string,
  commanders: string[],
  commandZoneConfiguration: CaseSemanticFixture["commandZoneConfiguration"],
): CaseSemanticFixture {
  const idx = DEV36_SCENARIO_CODES.indexOf(code);
  if (idx < 0) throw new Error(`Unknown dev scenario code: ${code}`);
  return buildFixtureForScenario(DEV36_INTERNAL_SCENARIOS[idx], caseId, commanders, commandZoneConfiguration);
}
