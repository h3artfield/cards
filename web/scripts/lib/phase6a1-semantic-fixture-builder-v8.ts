/**
 * DEV36 semantic fixture builder v8 — unified runtime schema, oracle clause evidence, role topology.
 */
...
export const RUNTIME_SCHEMA_V8 = "phase6a1-semantic-closure-runtime-input-v8";
...
function opaquePackageId(caseId: string, lens: Lens, index: number): string {
  return `pkg-${createHash("sha256").update(`${caseId}:${lens}:${index}`).digest("hex").slice(0, 4)}`;
}
...
export function buildDev36FixtureV8(
  ...
  const clauseRefs = clauseRefsForCommanders(catalog, commanders);
  ...
  for (const lens of Object.keys(fixture.lenses) as Lens[]) {
    ...
    lensData.hypotheses = lensData.hypotheses.map((h, hi) => ({
      ...
      commanderEvidenceRefs: lens === "DEPENDENT_SYNERGY" ? clauseRefs : [],
