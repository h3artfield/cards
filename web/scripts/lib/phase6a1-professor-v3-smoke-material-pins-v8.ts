/**
 * Korvold executable-stack import closure pins + identity verification (v8).
 */
...
import {
  ...
  type ProfessorV3CandidateExecutionAuthorizationV9,
} from "./phase6a1-professor-v3-smoke-execution-authorization-v9";
import {
...
export const PROFESSOR_V3_SMOKE_MATERIAL_PINS_V8_VERSION = "phase6a1-professor-v3-smoke-material-pins-v8";
export const PROFESSOR_V3_SMOKE_EXECUTION_IDENTITY_V8_PATH = resolve(
  MILESTONES,
  "phase6a1-professor-v3-smoke-execution-identity-v8-korvold-executable-v1.json",
);
export const PROFESSOR_V3_SMOKE_EXECUTION_PINS_V8_PATH = resolve(
  MILESTONES,
  "phase6a1-professor-v3-smoke-execution-pins-v8-korvold-executable-v1.json",
);

const WEB = resolve(REPO, "web");

export type ProfessorV3ReviewedExecutionIdentityV8 = {
...
export type ProfessorV3SmokeExecutionPinsV8 = {
  version: "phase6a1-professor-v3-smoke-execution-pins-v8-korvold-executable-v1";
  decision: string;
  sealedAt: string;
  stackIdentity: string;
  prospectiveCaseId: string;
...
/** Korvold executable runtime import closure — excludes external authorization roots (v8/v9). */
export const PROFESSOR_V3_SMOKE_MATERIAL_SOURCES_V8: ProfessorV3SmokeMaterialSourceV3[] = [
  ...
  { label: "run-phase6a1-seal-professor-v3-smoke-execution-identity-korvold-executable-v1", path: resolve(WEB, "scripts/run-phase6a1-seal-professor-v3-smoke-execution-identity-korvold-executable-v1.ts") },
  { label: "run-phase6a1-test-professor-v3-korvold-executable-stack-sealing-v1", path: resolve(WEB, "scripts/run-phase6a1-test-professor-v3-korvold-executable-stack-sealing-v1.ts") },
  { label: "run-phase6a1-report-professor-v3-korvold-executable-stack-sealing-v1", path: resolve(WEB, "scripts/run-phase6a1-report-professor-v3-korvold-executable-stack-sealing-v1.ts") },
  { label: "run-phase6a1-package-professor-v3-korvold-executable-stack-sealing-v1", path: resolve(WEB, "scripts/run-phase6a1-package-professor-v3-korvold-executable-stack-sealing-v1.ts") },
  { label: "phase6a1-professor-v3-korvold-prospective-smoke-execution-v1", path: resolve(WEB, "scripts/lib/phase6a1-professor-v3-korvold-prospective-smoke-execution-v1.ts") },
  { label: "phase6a1-professor-v3-korvold-model-request-boundary-v1", path: resolve(WEB, "scripts/lib/phase6a1-professor-v3-korvold-model-request-boundary-v1.ts") },
  { label: "phase6a1-professor-v3-korvold-orchestration-success-fixture-v1", path: resolve(WEB, "scripts/lib/phase6a1-professor-v3-korvold-orchestration-success-fixture-v1.ts") },
  { label: "phase6a1-professor-v3-smoke-material-pins-v8", path: resolve(WEB, "scripts/lib/phase6a1-professor-v3-smoke-material-pins-v8.ts") },
  ...
  { label: "phase6a1-professor-v3-smoke-failure-seal-v8", path: resolve(WEB, "scripts/lib/phase6a1-professor-v3-smoke-failure-seal-v8.ts") },
...
export function loadProfessorV3ReviewedExecutionIdentityV8(
  path: string = PROFESSOR_V3_SMOKE_EXECUTION_IDENTITY_V8_PATH,
  ...
  return JSON.parse(readFileSync(path, "utf8")) as ProfessorV3ReviewedExecutionIdentityV8;
}

export function computeProfessorV3SmokeMaterialPinsV8(args?: { sources?: ProfessorV3SmokeMaterialSourceV3[] }) {
  return computeProfessorV3SmokeMaterialPinsV3({ sources: args?.sources ?? PROFESSOR_V3_SMOKE_MATERIAL_SOURCES_V8 });
}

export function sealProfessorV3SmokeExecutionPinsV8(args?: {
  decision?: string;
  stackIdentity?: string;
  prospectiveCaseId?: string;
  mechanismTruthCaseId?: string;
  sources?: ProfessorV3SmokeMaterialSourceV3[];
}): ProfessorV3SmokeExecutionPinsV8 {
  const files = computeProfessorV3SmokeMaterialPinsV8({ sources: args?.sources });
  const dependencyManifestSha256 = computeDependencyManifestSha256V3(files);
  const payload = {
    version: "phase6a1-professor-v3-smoke-execution-pins-v8-korvold-executable-v1" as const,
    decision: args?.decision ?? PROFESSOR_V3_KORVOLD_PROSPECTIVE_SMOKE_DECISION_V1,
...
export function loadProfessorV3SmokeExecutionPinsV8(path: string = PROFESSOR_V3_SMOKE_EXECUTION_PINS_V8_PATH): ProfessorV3SmokeExecutionPinsV8 {
  if (!existsSync(path)) throw new Error(`Missing prospective execution pins: ${path}`);
  const pin = JSON.parse(readFileSync(path, "utf8")) as ProfessorV3SmokeExecutionPinsV8;
...
export type ProfessorV3SmokeExecutionIdentityVerificationV8 =
  | {
      ok: true;
      identity: ProfessorV3ReviewedExecutionIdentityV8;
      pins: ProfessorV3SmokeExecutionPinsV8;
...
export function verifyProfessorV3SmokeExecutionIdentityV8(args?: {
  pinsPath?: string;
  identityPath?: string;
  sources?: ProfessorV3SmokeMaterialSourceV3[];
  identity?: ProfessorV3ReviewedExecutionIdentityV8;
}): ProfessorV3SmokeExecutionIdentityVerificationV8 {
  const identity = args?.identity ?? loadProfessorV3ReviewedExecutionIdentityV8(args?.identityPath);
  const pinsPath = args?.pinsPath ?? PROFESSOR_V3_SMOKE_EXECUTION_PINS_V8_PATH;
  ...
  let pins: ProfessorV3SmokeExecutionPinsV8;
  try {
    pins = loadProfessorV3SmokeExecutionPinsV8(pinsPath);
  } catch (err) {
    return { ok: false, reason: "Prospective execution pins JSON invalid", identity, details: { pinsJson: String(err) } };
  }

  const current = computeProfessorV3SmokeMaterialPinsV8({ sources: args?.sources });
...
export function assertProfessorV3SmokeExecutionIdentityV8(args?: Parameters<typeof verifyProfessorV3SmokeExecutionIdentityV8>[0]) {
  const result = verifyProfessorV3SmokeExecutionIdentityV8(args);
  if (!result.ok) {
...
export function verifyProfessorV3SmokeExecutionAuthorizationV8(args?: {
  ...
  const identity = loadProfessorV3ReviewedExecutionIdentityV8(identityPath);
  ...
  const currentManifestSha = computeDependencyManifestSha256V3(computeProfessorV3SmokeMaterialPinsV8({ sources: args?.sources }));
...
export function verifyProfessorV3CandidateExecutionAuthorizationV9(args?: {
  authorization?: ProfessorV3CandidateExecutionAuthorizationV9;
  identityPath?: string;
  pinsPath?: string;
  sources?: ProfessorV3SmokeMaterialSourceV3[];
}): ProfessorV3SmokeExecutionAuthorizationVerificationV8 {
  return verifyProfessorV3SmokeExecutionAuthorizationV8({
    ...args,
    authorization: args?.authorization ?? PROFESSOR_V3_KORVOLD_PROSPECTIVE_SMOKE_CANDIDATE_EXECUTION_AUTHORIZATION_V9,
    identityPath: args?.identityPath ?? PROFESSOR_V3_SMOKE_EXECUTION_IDENTITY_V8_PATH,
    pinsPath: args?.pinsPath ?? PROFESSOR_V3_SMOKE_EXECUTION_PINS_V8_PATH,
  });
}

export function assertProfessorV3CandidateExecutionAuthorizationV9(
  args?: Parameters<typeof verifyProfessorV3CandidateExecutionAuthorizationV9>[0],
) {
  const result = verifyProfessorV3CandidateExecutionAuthorizationV9(args);
  if (!result.ok) {
    const detail = result.details ? Object.entries(result.details).map(([k, v]) => `${k}=${v}`).join("; ") : result.reason;
    throw new Error(`FAIL_CLOSED: Professor v3 Korvold candidate execution authorization mismatch — ${result.reason}: ${detail}`);
  }
  return result;
}

export function assertProfessorV3KorvoldExecutableSmokeExecutionPreflightV9(
  args?: Parameters<typeof verifyProfessorV3SmokeExecutionIdentityV8>[0] &
    Parameters<typeof verifyProfessorV3CandidateExecutionAuthorizationV9>[0],
) {
  assertProfessorV3CandidateExecutionAuthorizationV9(args);
  return assertProfessorV3SmokeExecutionIdentityV8({
    ...args,
    identityPath: args?.identityPath ?? PROFESSOR_V3_SMOKE_EXECUTION_IDENTITY_V8_PATH,
    pinsPath: args?.pinsPath ?? PROFESSOR_V3_SMOKE_EXECUTION_PINS_V8_PATH,
  });
...
export function buildProfessorV3KorvoldExecutableSmokeMaterialPinReportV1(
  identityVerification: Extract<ProfessorV3SmokeExecutionIdentityVerificationV8, { ok: true }>,
) {
  return {
    version: PROFESSOR_V3_SMOKE_MATERIAL_PINS_V8_VERSION,
