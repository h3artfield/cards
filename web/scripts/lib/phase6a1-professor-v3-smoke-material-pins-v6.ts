/**
 * Successor execution import closure pins + identity verification (v6).
 */
...
import {
  PROFESSOR_V3_INDEPENDENTLY_REVIEWED_EXECUTION_AUTHORIZATION_V7,
  type ProfessorV3IndependentlyReviewedExecutionAuthorizationV7,
} from "./phase6a1-professor-v3-smoke-execution-authorization-v7";
import {
...
export const PROFESSOR_V3_SMOKE_MATERIAL_PINS_V6_VERSION = "phase6a1-professor-v3-smoke-material-pins-v6";
export const PROFESSOR_V3_SMOKE_EXECUTION_IDENTITY_V6_PATH = resolve(
  MILESTONES,
  "phase6a1-professor-v3-smoke-execution-identity-v6-successor-v3.json",
);
export const PROFESSOR_V3_SMOKE_EXECUTION_PINS_V6_PATH = resolve(
  MILESTONES,
  "phase6a1-professor-v3-smoke-execution-pins-v6-successor-v3.json",
);

const WEB = resolve(REPO, "web");

export type ProfessorV3ReviewedExecutionIdentityV6 = {
...
export type ProfessorV3SmokeExecutionPinsV6 = {
  version: "phase6a1-professor-v3-smoke-execution-pins-v6-successor-v3";
  decision: string;
  sealedAt: string;
  stackIdentity: string;
  dependencyManifestSha256: string;
...
export const PROFESSOR_V3_SMOKE_MATERIAL_SOURCES_V6: ProfessorV3SmokeMaterialSourceV3[] = [
  { label: "run-phase6a1-execute-smoke-professor-v3-muldrotha-successor-v3", path: resolve(WEB, "scripts/run-phase6a1-execute-smoke-professor-v3-muldrotha-successor-v3.ts") },
  { label: "phase6a1-professor-v3-smoke-material-pins-v6", path: resolve(WEB, "scripts/lib/phase6a1-professor-v3-smoke-material-pins-v6.ts") },
  ...
  { label: "phase6a1-professor-v3-smoke-output-targets-v5", path: resolve(WEB, "scripts/lib/phase6a1-professor-v3-smoke-output-targets-v5.ts") },
...
export function loadProfessorV3ReviewedExecutionIdentityV6(
  path: string = PROFESSOR_V3_SMOKE_EXECUTION_IDENTITY_V6_PATH,
  ...
  return JSON.parse(readFileSync(path, "utf8")) as ProfessorV3ReviewedExecutionIdentityV6;
}

export function computeProfessorV3SmokeMaterialPinsV6(args?: { sources?: ProfessorV3SmokeMaterialSourceV3[] }) {
  return computeProfessorV3SmokeMaterialPinsV3({ sources: args?.sources ?? PROFESSOR_V3_SMOKE_MATERIAL_SOURCES_V6 });
}

export function sealProfessorV3SmokeExecutionPinsV6(args?: {
  decision?: string;
  stackIdentity?: string;
  sources?: ProfessorV3SmokeMaterialSourceV3[];
}): ProfessorV3SmokeExecutionPinsV6 {
  const files = computeProfessorV3SmokeMaterialPinsV6({ sources: args?.sources });
  const dependencyManifestSha256 = computeDependencyManifestSha256V3(files);
  const payload = {
    version: "phase6a1-professor-v3-smoke-execution-pins-v6-successor-v3" as const,
    decision:
      args?.decision ??
      "PROFESSOR_V3_INCOMPLETE_RESPONSE_AND_OUTPUT_BUDGET_REPAIR_V1_AUTHORIZED_NO_MODEL",
    sealedAt: new Date().toISOString(),
    stackIdentity: args?.stackIdentity ?? "professor-v3-muldrotha-smoke-execution-tree-v6-successor-v3",
...
export function loadProfessorV3SmokeExecutionPinsV6(path: string = PROFESSOR_V3_SMOKE_EXECUTION_PINS_V6_PATH): ProfessorV3SmokeExecutionPinsV6 {
  if (!existsSync(path)) throw new Error(`Missing successor execution pins: ${path}`);
  const pin = JSON.parse(readFileSync(path, "utf8")) as ProfessorV3SmokeExecutionPinsV6;
...
export type ProfessorV3SmokeExecutionIdentityVerificationV6 =
  | {
      ok: true;
      identity: ProfessorV3ReviewedExecutionIdentityV6;
      pins: ProfessorV3SmokeExecutionPinsV6;
...
export function verifyProfessorV3SmokeExecutionIdentityV6(args?: {
  pinsPath?: string;
  identityPath?: string;
  sources?: ProfessorV3SmokeMaterialSourceV3[];
  identity?: ProfessorV3ReviewedExecutionIdentityV6;
}): ProfessorV3SmokeExecutionIdentityVerificationV6 {
  const identity = args?.identity ?? loadProfessorV3ReviewedExecutionIdentityV6(args?.identityPath);
  const pinsPath = args?.pinsPath ?? PROFESSOR_V3_SMOKE_EXECUTION_PINS_V6_PATH;
  ...
  let pins: ProfessorV3SmokeExecutionPinsV6;
  try {
    pins = loadProfessorV3SmokeExecutionPinsV6(pinsPath);
  } catch (err) {
    return { ok: false, reason: "Successor execution pins JSON invalid", identity, details: { pinsJson: String(err) } };
  }

  const current = computeProfessorV3SmokeMaterialPinsV6({ sources: args?.sources });
...
export function assertProfessorV3SmokeExecutionIdentityV6(args?: Parameters<typeof verifyProfessorV3SmokeExecutionIdentityV6>[0]) {
  const result = verifyProfessorV3SmokeExecutionIdentityV6(args);
  if (!result.ok) {
...
export type ProfessorV3SmokeExecutionAuthorizationVerificationV7 =
  | { ok: true; authorization: ProfessorV3IndependentlyReviewedExecutionAuthorizationV7 }
  | {
      ok: false;
      reason: string;
      authorization: ProfessorV3IndependentlyReviewedExecutionAuthorizationV7;
      details?: Record<string, string>;
    };

export function verifyProfessorV3SmokeExecutionAuthorizationV7(args?: {
  authorization?: ProfessorV3IndependentlyReviewedExecutionAuthorizationV7;
  identityPath?: string;
  pinsPath?: string;
  sources?: ProfessorV3SmokeMaterialSourceV3[];
}): ProfessorV3SmokeExecutionAuthorizationVerificationV7 {
  const authorization = args?.authorization ?? PROFESSOR_V3_INDEPENDENTLY_REVIEWED_EXECUTION_AUTHORIZATION_V7;
  const identityPath = args?.identityPath ?? PROFESSOR_V3_SMOKE_EXECUTION_IDENTITY_V6_PATH;
  const pinsPath = args?.pinsPath ?? PROFESSOR_V3_SMOKE_EXECUTION_PINS_V6_PATH;
  ...
  const identity = loadProfessorV3ReviewedExecutionIdentityV6(identityPath);
  ...
  const currentManifestSha = computeDependencyManifestSha256V3(computeProfessorV3SmokeMaterialPinsV6({ sources: args?.sources }));
...
export function assertProfessorV3SmokeExecutionAuthorizationV7(args?: Parameters<typeof verifyProfessorV3SmokeExecutionAuthorizationV7>[0]) {
  const result = verifyProfessorV3SmokeExecutionAuthorizationV7(args);
  if (!result.ok) {
    const detail = result.details ? Object.entries(result.details).map(([k, v]) => `${k}=${v}`).join("; ") : result.reason;
...
export function assertProfessorV3SuccessorSmokeExecutionPreflightV7(args?: Parameters<typeof verifyProfessorV3SmokeExecutionIdentityV6>[0] & Parameters<typeof verifyProfessorV3SmokeExecutionAuthorizationV7>[0]) {
  assertProfessorV3SmokeExecutionAuthorizationV7(args);
  return assertProfessorV3SmokeExecutionIdentityV6(args);
}

export function buildProfessorV3SuccessorSmokeMaterialPinReportV2(
  identityVerification: Extract<ProfessorV3SmokeExecutionIdentityVerificationV6, { ok: true }>,
) {
  return {
    version: PROFESSOR_V3_SMOKE_MATERIAL_PINS_V6_VERSION,
