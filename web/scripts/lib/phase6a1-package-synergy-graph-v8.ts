/**
 * Package synergy graph v8 — v7 Hua Tuo fix preserved; atomic conjuncts, cardinality, harmony edge aggregation.
 */
import type { SemanticPackage } from "../../src/lib/deck-synthesis/professor-planning-contracts-v2";
import {
  CANONICAL_RESOURCE_ONTOLOGY_V6_VERSION,
...
} from "./phase6a1-canonical-resource-ontology-v6";

export const PACKAGE_SYNERGY_GRAPH_V8_VERSION = "phase6a1-package-synergy-graph-v8";
...
export type PackageSynergyTypedEdge = {
  ...
  supportPointers?: EdgeSupportPointer[];
};
...
function edgeKey(edge: PackageSynergyTypedEdge): string {
  return [edge.fromPackageId, edge.toPackageId, edge.kind, edge.resource].join("|");
}

export function semanticEdgeKey(edge: PackageSynergyTypedEdge): string {
  return edgeKey(edge);
}

function supportPointerKey(support: EdgeSupportPointer): string {
  return [
    support.producerField,
    support.producerIndex,
    support.producerText,
    support.consumerField,
    support.consumerIndex,
    support.consumerText,
  ].join("|");
}

/** Collapse repeated package-pair/kind/resource evidence before Harmony scoring. */
export function aggregateSemanticEdgesForHarmony(edges: PackageSynergyTypedEdge[]): PackageSynergyTypedEdge[] {
  const map = new Map<string, PackageSynergyTypedEdge>();
  for (const edge of edges) {
    const key = edgeKey(edge);
    const existing = map.get(key);
    if (!existing) {
      map.set(key, { ...edge, supportPointers: [edge.support] });
      continue;
    }
    const pointers = existing.supportPointers ?? [existing.support];
    const next = edge.supportPointers ?? [edge.support];
    for (const pointer of next) {
      if (!pointers.some((p) => supportPointerKey(p) === supportPointerKey(pointer))) {
        pointers.push(pointer);
      }
    }
    existing.supportPointers = pointers;
  }
  return [...map.values()];
}
...
function runSemanticRegressions(
  ...
  if (caseId === "blindv5-29-static-restriction") {
    ...
    const mustMatchTopLibrary = causal.some(
      (e) =>
        /multiple recovered creatures ordered on top of the library/i.test(e.support.producerText) &&
        /creature card placed on top of the library/i.test(e.support.consumerText),
    );
    assertions.push({
      id: "HUA_TUO_TOP_LIBRARY_RECOVERED_MUST_MATCH",
      passed: mustMatchTopLibrary,
      detail: mustMatchTopLibrary
        ? "Explicit top-library recovered creature satisfies top-library requirement"
        : "Missing valid top-library recovered-creature causal edge",
    });
    const mustNotMatchAccessible = causal.some(
      (e) =>
        /recovered creatures in accessible zones/i.test(e.support.producerText) &&
        /creature card placed on top of the library/i.test(e.support.consumerText),
    );
    assertions.push({
      id: "HUA_TUO_ACCESSIBLE_RECOVERED_MUST_NOT_MATCH_TOP_LIBRARY",
      passed: !mustNotMatchAccessible,
      detail: mustNotMatchAccessible
        ? "Accessible/unspecified recovered creature incorrectly satisfies top-library requirement"
        : "Accessible recovery does not satisfy top-library requirement",
    });
  }

  if (caseId === "blindv5-11-commander-background") {
    const badCompound = causal.some(
      (e) =>
        /recovered mana development/i.test(e.support.producerText) &&
        /mana development and interaction/i.test(e.support.consumerText),
    );
    assertions.push({
      id: "ERINIS_RECOVERED_MANA_NOT_COMPOUND_MANA_AND_INTERACTION",
      passed: !badCompound,
      detail: badCompound
        ? "Recovered mana development incorrectly satisfies compound mana-and-interaction requirement"
        : "Compound conjunctive requirement not partially satisfied as full match",
    });
    const mustMatchManaAtom = causal.some(
      (e) =>
        /recovered mana development/i.test(e.support.producerText) &&
        /^mana development$/i.test(e.support.consumerText.trim()),
    );
    assertions.push({
      id: "ERINIS_RECOVERED_MANA_MAY_MATCH_MANA_DEVELOPMENT_ATOM",
      passed: mustMatchManaAtom,
      detail: mustMatchManaAtom
        ? "Recovered mana development may satisfy the MANA_DEVELOPMENT conjunct only"
        : "Missing valid recovered-mana to mana-development atom edge",
    });
  }

  if (caseId === "blindv5-59-tutor-toolbox") {
    const badSingularMultiple = causal.some(
      (e) =>
        /a power-4-plus creature when a threshold-compatible target is chosen/i.test(e.support.producerText) &&
        /multiple power-4-plus creatures/i.test(e.support.consumerText),
    );
    assertions.push({
      id: "TOOLBOX_SINGULAR_POWER4_NOT_MULTIPLE_POWER4",
      passed: !badSingularMultiple,
      detail: badSingularMultiple
        ? "Singular power-4 creature incorrectly satisfies multiple power-4 requirement"
        : "Singular producer does not satisfy MULTIPLE cardinality requirement",
    });
...
export type PackageSynergyGraphV8 = {
...
export function causalEdgeRecordKey(edge: PackageSynergyTypedEdge): string {
  return [
    edge.kind,
    edge.fromPackageId,
    edge.toPackageId,
    edge.resource,
    edge.support.producerText,
    edge.support.consumerText,
  ].join("|");
}

const ALLOWED_V7_TO_V8_CAUSAL_REMOVALS = new Set([
  "PRODUCES_FOR|H1-P3|H3-P3|BOUNDED_MANA|Recovered mana development|Mana development and interaction",
  "PRODUCES_FOR|H3-P2|H3-P3|BOUNDED_MANA|Recovered mana development|Mana development and interaction",
  "REQUIRES_FROM|H3-P3|H1-P3|BOUNDED_MANA|Recovered mana development|Mana development and interaction",
  "REQUIRES_FROM|H3-P3|H3-P2|BOUNDED_MANA|Recovered mana development|Mana development and interaction",
  "PRODUCES_FOR|pkg-power4-utility-toolbox|pkg-qualifying-attacker-density|POWER_FOUR_PLUS_ATTACKER|A power-4-plus creature when a threshold-compatible target is chosen|Multiple power-4-plus creatures",
  "REQUIRES_FROM|pkg-qualifying-attacker-density|pkg-power4-utility-toolbox|POWER_FOUR_PLUS_ATTACKER|A power-4-plus creature when a threshold-compatible target is chosen|Multiple power-4-plus creatures",
]);

const ALLOWED_V7_TO_V8_CAUSAL_ADDITIONS = new Set([
  "PRODUCES_FOR|H1-P3|H3-P3|BOUNDED_MANA|Recovered mana development|Mana development",
  "PRODUCES_FOR|H3-P2|H3-P3|BOUNDED_MANA|Recovered mana development|Mana development",
  "REQUIRES_FROM|H3-P3|H1-P3|BOUNDED_MANA|Recovered mana development|Mana development",
  "REQUIRES_FROM|H3-P3|H3-P2|BOUNDED_MANA|Recovered mana development|Mana development",
]);

export function auditCausalPredecessorDiff(
  predecessorCausal: PackageSynergyTypedEdge[],
  currentCausal: PackageSynergyTypedEdge[],
): {
  passed: boolean;
  ...
  const predecessorKeys = new Set(predecessorCausal.map(causalEdgeRecordKey));
  const currentKeys = new Set(currentCausal.map(causalEdgeRecordKey));
  const removed = [...predecessorKeys].filter((key) => !currentKeys.has(key)).sort();
  const added = [...currentKeys].filter((key) => !predecessorKeys.has(key)).sort();
  const unexpectedRemoved = removed.filter((key) => !ALLOWED_V7_TO_V8_CAUSAL_REMOVALS.has(key));
  const unexpectedAdded = added.filter((key) => !ALLOWED_V7_TO_V8_CAUSAL_ADDITIONS.has(key));
  const passed =
    unexpectedRemoved.length === 0 &&
    unexpectedAdded.length === 0 &&
    removed.length === ALLOWED_V7_TO_V8_CAUSAL_REMOVALS.size &&
    added.length === ALLOWED_V7_TO_V8_CAUSAL_ADDITIONS.size;
  return {
    passed,
    predecessorCount: predecessorKeys.size,
...
export function buildPackageSynergyGraphV8(caseId: string, packages: SemanticPackage[]): PackageSynergyGraphV8 {
  ...
  const qa: SynergyGraphQA = {
    version: PACKAGE_SYNERGY_GRAPH_V8_VERSION,
    ontologyVersion: CANONICAL_RESOURCE_ONTOLOGY_V6_VERSION,
...
export function enrichPackagesWithSynergyGraphV8(
  caseId: string,
  packages: SemanticPackage[],
): {
  packages: SemanticPackage[];
  ...
  const graph = buildPackageSynergyGraphV8(caseId, clean);
