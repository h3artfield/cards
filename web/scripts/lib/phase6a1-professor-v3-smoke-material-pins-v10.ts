/**
 * Zada executable-stack import closure pins + identity verification (v10).
 */
...
} from "./phase6a1-professor-v3-smoke-execution-authorization-zada-v1";
import {
...
import { ZADA_PROSPECTIVE_MECHANISM_TRUTH_V1_PATH } from "./phase6a1-spent-pilot-truth-loader-v1";

export const PROFESSOR_V3_SMOKE_MATERIAL_PINS_V10_VERSION = "phase6a1-professor-v3-smoke-material-pins-v10";
export const PROFESSOR_V3_SMOKE_EXECUTION_IDENTITY_V10_PATH = resolve(
  MILESTONES,
  "phase6a1-professor-v3-smoke-execution-identity-v10-zada-executable-v1.json",
);
export const PROFESSOR_V3_SMOKE_EXECUTION_PINS_V10_PATH = resolve(
  MILESTONES,
  "phase6a1-professor-v3-smoke-execution-pins-v10-zada-executable-v1.json",
);

const WEB = resolve(REPO, "web");

export type ProfessorV3ReviewedExecutionIdentityV10 = {
...
export type ProfessorV3SmokeExecutionPinsV10 = {
  version: "phase6a1-professor-v3-smoke-execution-pins-v10-zada-executable-v1";
  decision: string;
  sealedAt: string;
  stackIdentity: string;
  prospectiveCaseId: string;
...
/** Zada executable runtime import closure — excludes patched candidate authorization anchor file. */
export const PROFESSOR_V3_SMOKE_MATERIAL_SOURCES_V10: ProfessorV3SmokeMaterialSourceV3[] = [
  { label: "run-phase6a1-execute-smoke-professor-v3-zada-prospective-v1", path: resolve(WEB, "scripts/run-phase6a1-execute-smoke-professor-v3-zada-prospective-v1.ts") },
  { label: "run-phase6a1-seal-professor-v3-smoke-execution-identity-zada-executable-v1", path: resolve(WEB, "scripts/run-phase6a1-seal-professor-v3-smoke-execution-identity-zada-executable-v1.ts") },
  { label: "run-phase6a1-test-professor-v3-zada-executable-stack-sealing-v1", path: resolve(WEB, "scripts/run-phase6a1-test-professor-v3-zada-executable-stack-sealing-v1.ts") },
  { label: "run-phase6a1-package-professor-v3-zada-executable-stack-finalization-v1", path: resolve(WEB, "scripts/run-phase6a1-package-professor-v3-zada-executable-stack-finalization-v1.ts") },
  { label: "phase6a1-professor-v3-zada-prospective-smoke-execution-v1", path: resolve(WEB, "scripts/lib/phase6a1-professor-v3-zada-prospective-smoke-execution-v1.ts") },
  { label: "phase6a1-professor-v3-zada-model-request-boundary-v1", path: resolve(WEB, "scripts/lib/phase6a1-professor-v3-zada-model-request-boundary-v1.ts") },
  { label: "phase6a1-professor-v3-zada-pre-model-audit-v1", path: resolve(WEB, "scripts/lib/phase6a1-professor-v3-zada-pre-model-audit-v1.ts") },
  { label: "phase6a1-professor-v3-smoke-material-pins-v10", path: resolve(WEB, "scripts/lib/phase6a1-professor-v3-smoke-material-pins-v10.ts") },
  { label: "phase6a1-professor-v3-model-call-budget-v1", path: resolve(WEB, "scripts/lib/phase6a1-professor-v3-model-call-budget-v1.ts") },
  { label: "phase6a1-professor-v3-smoke-terminal-seal-v10", path: resolve(WEB, "scripts/lib/phase6a1-professor-v3-smoke-terminal-seal-v10.ts") },
  ...
  { label: "grounding-derivation-graph-v1", path: resolve(WEB, "src/lib/deck-synthesis/grounding-derivation-graph-v1.ts") },
  ...
  { label: "phase6a1-professor-v3-zada-prospective-mechanism-truth-v1", path: ZADA_PROSPECTIVE_MECHANISM_TRUTH_V1_PATH },
];

export function loadProfessorV3ReviewedExecutionIdentityV10(
  path: string = PROFESSOR_V3_SMOKE_EXECUTION_IDENTITY_V10_PATH,
): ProfessorV3ReviewedExecutionIdentityV10 {
  if (!existsSync(path)) throw new Error(`Missing reviewed prospective execution identity artifact: ${path}`);
  return JSON.parse(readFileSync(path, "utf8")) as ProfessorV3ReviewedExecutionIdentityV10;
}

export function computeProfessorV3SmokeMaterialPinsV10(args?: { sources?: ProfessorV3SmokeMaterialSourceV3[] }) {
  return computeProfessorV3SmokeMaterialPinsV3({ sources: args?.sources ?? PROFESSOR_V3_SMOKE_MATERIAL_SOURCES_V10 });
}

export function sealProfessorV3SmokeExecutionPinsV10(args?: {
  decision?: string;
  stackIdentity?: string;
  prospectiveCaseId?: string;
  mechanismTruthCaseId?: string;
  sources?: ProfessorV3SmokeMaterialSourceV3[];
}): ProfessorV3SmokeExecutionPinsV10 {
  const files = computeProfessorV3SmokeMaterialPinsV10({ sources: args?.sources });
  const dependencyManifestSha256 = computeDependencyManifestSha256V3(files);
  const payload = {
    version: "phase6a1-professor-v3-smoke-execution-pins-v10-zada-executable-v1" as const,
    decision: args?.decision ?? PROFESSOR_V3_ZADA_PROSPECTIVE_SMOKE_DECISION_V1,
    sealedAt: new Date().toISOString(),
    stackIdentity: args?.stackIdentity ?? "professor-v3-zada-smoke-execution-tree-v1-prospective",
    prospectiveCaseId: args?.prospectiveCaseId ?? "professor-v3-smoke-zada-prospective-v1",
    mechanismTruthCaseId: args?.mechanismTruthCaseId ?? "multi-zada",
...
export function loadProfessorV3SmokeExecutionPinsV10(path: string = PROFESSOR_V3_SMOKE_EXECUTION_PINS_V10_PATH): ProfessorV3SmokeExecutionPinsV10 {
  if (!existsSync(path)) throw new Error(`Missing prospective execution pins: ${path}`);
  const pin = JSON.parse(readFileSync(path, "utf8")) as ProfessorV3SmokeExecutionPinsV10;
...
export type ProfessorV3SmokeExecutionIdentityVerificationV10 =
  | {
      ok: true;
      identity: ProfessorV3ReviewedExecutionIdentityV10;
      pins: ProfessorV3SmokeExecutionPinsV10;
...
export function verifyProfessorV3SmokeExecutionIdentityV10(args?: {
  pinsPath?: string;
  identityPath?: string;
  sources?: ProfessorV3SmokeMaterialSourceV3[];
  identity?: ProfessorV3ReviewedExecutionIdentityV10;
}): ProfessorV3SmokeExecutionIdentityVerificationV10 {
  const identity = args?.identity ?? loadProfessorV3ReviewedExecutionIdentityV10(args?.identityPath);
  const pinsPath = args?.pinsPath ?? PROFESSOR_V3_SMOKE_EXECUTION_PINS_V10_PATH;
  ...
  let pins: ProfessorV3SmokeExecutionPinsV10;
  try {
    pins = loadProfessorV3SmokeExecutionPinsV10(pinsPath);
  } catch (err) {
    return { ok: false, reason: "Prospective execution pins JSON invalid", identity, details: { pinsJson: String(err) } };
  }

  const current = computeProfessorV3SmokeMaterialPinsV10({ sources: args?.sources });
...
export function assertProfessorV3SmokeExecutionIdentityV10(args?: Parameters<typeof verifyProfessorV3SmokeExecutionIdentityV10>[0]) {
  const result = verifyProfessorV3SmokeExecutionIdentityV10(args);
  if (!result.ok) {
...
export function verifyProfessorV3SmokeExecutionAuthorizationV10(args?: {
  ...
  const authorization = args?.authorization ?? PROFESSOR_V3_ZADA_PROSPECTIVE_SMOKE_CANDIDATE_EXECUTION_AUTHORIZATION_V1;
  ...
  const identity = loadProfessorV3ReviewedExecutionIdentityV10(identityPath);
  ...
  const currentManifestSha = computeDependencyManifestSha256V3(computeProfessorV3SmokeMaterialPinsV10({ sources: args?.sources }));
...
export function verifyProfessorV3CandidateExecutionAuthorizationV11(args?: {
  ...
  const result = verifyProfessorV3SmokeExecutionAuthorizationV10({
    ...args,
    authorization,
    identityPath: args?.identityPath ?? PROFESSOR_V3_SMOKE_EXECUTION_IDENTITY_V10_PATH,
    pinsPath: args?.pinsPath ?? PROFESSOR_V3_SMOKE_EXECUTION_PINS_V10_PATH,
  });
...
export function assertProfessorV3CandidateExecutionAuthorizationV11(
  ...
  if (!result.ok) {
    const detail = result.details ? Object.entries(result.details).map(([k, v]) => `${k}=${v}`).join("; ") : result.reason;
    throw new Error(`FAIL_CLOSED: Professor v3 Zada candidate execution authorization mismatch — ${result.reason}: ${detail}`);
  }
  return result;
}

export function assertProfessorV3ZadaExecutableSmokeExecutionPreflightV11(
  args?: Parameters<typeof verifyProfessorV3SmokeExecutionIdentityV10>[0] &
  ...
  return assertProfessorV3SmokeExecutionIdentityV10({
    ...args,
    identityPath: args?.identityPath ?? PROFESSOR_V3_SMOKE_EXECUTION_IDENTITY_V10_PATH,
    pinsPath: args?.pinsPath ?? PROFESSOR_V3_SMOKE_EXECUTION_PINS_V10_PATH,
  });
...
export function buildProfessorV3ZadaExecutableSmokeMaterialPinReportV1(
  identityVerification: Extract<ProfessorV3SmokeExecutionIdentityVerificationV10, { ok: true }>,
) {
  return {
    version: PROFESSOR_V3_SMOKE_MATERIAL_PINS_V10_VERSION,
