/**
 * Successor execution import closure pins + identity verification (v5).
 */
...
import {
  PROFESSOR_V3_INDEPENDENTLY_REVIEWED_EXECUTION_AUTHORIZATION_V6,
  type ProfessorV3IndependentlyReviewedExecutionAuthorizationV6,
} from "./phase6a1-professor-v3-smoke-execution-authorization-v6";
import {
...
export const PROFESSOR_V3_SMOKE_MATERIAL_PINS_V5_VERSION = "phase6a1-professor-v3-smoke-material-pins-v5";
export const PROFESSOR_V3_SMOKE_EXECUTION_IDENTITY_V5_PATH = resolve(
  MILESTONES,
  "phase6a1-professor-v3-smoke-execution-identity-v5-successor-v2.json",
);
export const PROFESSOR_V3_SMOKE_EXECUTION_PINS_V5_PATH = resolve(
  MILESTONES,
  "phase6a1-professor-v3-smoke-execution-pins-v5-successor-v2.json",
);

const WEB = resolve(REPO, "web");

export type ProfessorV3ReviewedExecutionIdentityV5 = {
...
export type ProfessorV3SmokeExecutionPinsV5 = {
  version: "phase6a1-professor-v3-smoke-execution-pins-v5-successor-v2";
  decision: string;
  sealedAt: string;
  stackIdentity: string;
  dependencyManifestSha256: string;
...
export const PROFESSOR_V3_SMOKE_MATERIAL_SOURCES_V5: ProfessorV3SmokeMaterialSourceV3[] = [
  { label: "run-phase6a1-execute-smoke-professor-v3-muldrotha-successor-v2", path: resolve(WEB, "scripts/run-phase6a1-execute-smoke-professor-v3-muldrotha-successor-v2.ts") },
  { label: "phase6a1-professor-v3-smoke-material-pins-v5", path: resolve(WEB, "scripts/lib/phase6a1-professor-v3-smoke-material-pins-v5.ts") },
  { label: "phase6a1-professor-v3-smoke-failure-seal-v6", path: resolve(WEB, "scripts/lib/phase6a1-professor-v3-smoke-failure-seal-v6.ts") },
  { label: "phase6a1-professor-v3-smoke-output-targets-v4", path: resolve(WEB, "scripts/lib/phase6a1-professor-v3-smoke-output-targets-v4.ts") },
...
export function loadProfessorV3ReviewedExecutionIdentityV5(
  path: string = PROFESSOR_V3_SMOKE_EXECUTION_IDENTITY_V5_PATH,
): ProfessorV3ReviewedExecutionIdentityV5 {
  if (!existsSync(path)) throw new Error(`Missing reviewed successor execution identity artifact: ${path}`);
  return JSON.parse(readFileSync(path, "utf8")) as ProfessorV3ReviewedExecutionIdentityV5;
}

export function computeProfessorV3SmokeMaterialPinsV5(args?: { sources?: ProfessorV3SmokeMaterialSourceV3[] }) {
  return computeProfessorV3SmokeMaterialPinsV3({ sources: args?.sources ?? PROFESSOR_V3_SMOKE_MATERIAL_SOURCES_V5 });
}

export function sealProfessorV3SmokeExecutionPinsV5(args?: {
  decision?: string;
  stackIdentity?: string;
  sources?: ProfessorV3SmokeMaterialSourceV3[];
}): ProfessorV3SmokeExecutionPinsV5 {
  const files = computeProfessorV3SmokeMaterialPinsV5({ sources: args?.sources });
  const dependencyManifestSha256 = computeDependencyManifestSha256V3(files);
  const payload = {
    version: "phase6a1-professor-v3-smoke-execution-pins-v5-successor-v2" as const,
    decision:
      args?.decision ??
      "PROFESSOR_V3_SUCCESSOR_SMOKE_V1_SPENT_FAIL_API_SCHEMA_TYPE_ANNOTATION_REPAIR_REQUIRED_NO_MODEL",
    sealedAt: new Date().toISOString(),
    stackIdentity: args?.stackIdentity ?? "professor-v3-muldrotha-smoke-execution-tree-v5-successor-v2",
...
export function loadProfessorV3SmokeExecutionPinsV5(path: string = PROFESSOR_V3_SMOKE_EXECUTION_PINS_V5_PATH): ProfessorV3SmokeExecutionPinsV5 {
  if (!existsSync(path)) throw new Error(`Missing successor execution pins: ${path}`);
  const pin = JSON.parse(readFileSync(path, "utf8")) as ProfessorV3SmokeExecutionPinsV5;
...
export type ProfessorV3SmokeExecutionIdentityVerificationV5 =
  | {
      ok: true;
      identity: ProfessorV3ReviewedExecutionIdentityV5;
      pins: ProfessorV3SmokeExecutionPinsV5;
...
export function verifyProfessorV3SmokeExecutionIdentityV5(args?: {
  pinsPath?: string;
  identityPath?: string;
  sources?: ProfessorV3SmokeMaterialSourceV3[];
  identity?: ProfessorV3ReviewedExecutionIdentityV5;
}): ProfessorV3SmokeExecutionIdentityVerificationV5 {
  const identity = args?.identity ?? loadProfessorV3ReviewedExecutionIdentityV5(args?.identityPath);
  const pinsPath = args?.pinsPath ?? PROFESSOR_V3_SMOKE_EXECUTION_PINS_V5_PATH;
  ...
  let pins: ProfessorV3SmokeExecutionPinsV5;
  try {
    pins = loadProfessorV3SmokeExecutionPinsV5(pinsPath);
  } catch (err) {
    return { ok: false, reason: "Successor execution pins JSON invalid", identity, details: { pinsJson: String(err) } };
  }

  const current = computeProfessorV3SmokeMaterialPinsV5({ sources: args?.sources });
...
export function assertProfessorV3SmokeExecutionIdentityV5(args?: Parameters<typeof verifyProfessorV3SmokeExecutionIdentityV5>[0]) {
  const result = verifyProfessorV3SmokeExecutionIdentityV5(args);
  if (!result.ok) {
...
export type ProfessorV3SmokeExecutionAuthorizationVerificationV5 =
  | { ok: true; authorization: ProfessorV3IndependentlyReviewedExecutionAuthorizationV6 }
  | {
      ok: false;
      reason: string;
      authorization: ProfessorV3IndependentlyReviewedExecutionAuthorizationV6;
      details?: Record<string, string>;
    };

export function verifyProfessorV3SmokeExecutionAuthorizationV6(args?: {
  authorization?: ProfessorV3IndependentlyReviewedExecutionAuthorizationV6;
  identityPath?: string;
  pinsPath?: string;
  sources?: ProfessorV3SmokeMaterialSourceV3[];
}): ProfessorV3SmokeExecutionAuthorizationVerificationV5 {
  const authorization = args?.authorization ?? PROFESSOR_V3_INDEPENDENTLY_REVIEWED_EXECUTION_AUTHORIZATION_V6;
  const identityPath = args?.identityPath ?? PROFESSOR_V3_SMOKE_EXECUTION_IDENTITY_V5_PATH;
  const pinsPath = args?.pinsPath ?? PROFESSOR_V3_SMOKE_EXECUTION_PINS_V5_PATH;
  ...
  const identity = loadProfessorV3ReviewedExecutionIdentityV5(identityPath);
  ...
  const currentManifestSha = computeDependencyManifestSha256V3(computeProfessorV3SmokeMaterialPinsV5({ sources: args?.sources }));
...
export function assertProfessorV3SmokeExecutionAuthorizationV6(args?: Parameters<typeof verifyProfessorV3SmokeExecutionAuthorizationV6>[0]) {
  const result = verifyProfessorV3SmokeExecutionAuthorizationV6(args);
  if (!result.ok) {
    const detail = result.details ? Object.entries(result.details).map(([k, v]) => `${k}=${v}`).join("; ") : result.reason;
...
export function assertProfessorV3SuccessorSmokeExecutionPreflightV6(args?: Parameters<typeof verifyProfessorV3SmokeExecutionIdentityV5>[0] & Parameters<typeof verifyProfessorV3SmokeExecutionAuthorizationV6>[0]) {
  assertProfessorV3SmokeExecutionAuthorizationV6(args);
  return assertProfessorV3SmokeExecutionIdentityV5(args);
}

export function buildProfessorV3SuccessorSmokeMaterialPinReportV2(
  identityVerification: Extract<ProfessorV3SmokeExecutionIdentityVerificationV5, { ok: true }>,
) {
  return {
    version: PROFESSOR_V3_SMOKE_MATERIAL_PINS_V5_VERSION,
