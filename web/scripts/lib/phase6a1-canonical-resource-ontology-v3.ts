/**
 * Canonical typed resource ontology v3 — semantic-role compatibility for causal edges.
 */
import type { SemanticPackage } from "../../src/lib/deck-synthesis/professor-planning-contracts-v2";

export const CANONICAL_RESOURCE_ONTOLOGY_V3_VERSION = "phase6a1-canonical-resource-ontology-v3";
...
function classifyLine(text: string, field: ClassifiedResource["field"], index: number): ClassifiedResource[] {
  ...
  if (/multiple recovered creatures ordered|recovered creatures ordered on top/.test(t)) {
  ...
  if (/repeatable nonland mana base|developed nonland mana base|nonland permanent slots/.test(t)) {
    push("NONLAND_MANA_PERMANENT", "SOURCE_OBJECT");
    return out;
  }
  ...
  if (/top-card access|draw capacity|topdeck access|top of the library/.test(t)) {
  ...
  if (/elf|tap-abilit|tap ability|hand-to-board|conversion outlet|noncreature card selection|inexpensive noncreature/.test(t)) {
