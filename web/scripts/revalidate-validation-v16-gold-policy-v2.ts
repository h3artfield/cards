/**
 * Revalidate v16 with gold-policy-validator v1.5 (created-object plural detection).
 * Preserves pre-v1.5 certified artifact; corrects gold parser-blind if violations found.
 *
 * Run: cd web && npx tsx scripts/revalidate-validation-v16-gold-policy-v2.ts
 */
...
const PRESERVED_HASH = "f62c7753abbc2ed11f932ac303caa924337bf3977597e40ac0e311da7172ba2c";
...
function main() {
  ...
  const preservedCertPath = resolve(OUT_DIR, "validation-v16-gold-policy-certificate-v1.json");
  const preservedImmutable = writeImmutableBenchmarkCopy({
    setVersion: SET_VERSION,
    phase: "certified",
    contentHash: PRESERVED_HASH,
    sourcePath: V16_PATH,
    outDir: OUT_DIR,
  });
  ...
  copyFileSync(
    resolve(V16_PATH),
    resolve(OUT_DIR, `oracle-action-eval-validation-v16-certified-v1.4-${PRESERVED_HASH.slice(0, 16)}.json`),
  );
  ...
  const correctedCases = envelope.cases.map((c) => {
    ...
    return attachCaseScopeV11({
      ...
      goldReviewVersion: "validation-v16-gold-policy-corrector-v2",
      goldReviewedAt: new Date().toISOString(),
      parserConsulted: false,
      prePolicyValidation: {
        preservedContentHash: PRESERVED_HASH,
        preservedArtifact: preservedImmutable.replace(/\\/g, "/"),
        priorCertificate: "validation-v16-gold-policy-certificate-v1.json",
        status: "superseded_before_parser_contact",
        correctedAt: new Date().toISOString(),
        corrector: "validation-v16-gold-policy-corrector-v2",
        policyStackUpgrade: "semantic-gold-policy-v1.5",
      },
  ...
  const postValidation = validateBenchmarkGoldPolicy({
    cases: correctedCases,
    benchmarkHash: correctedEnvelope.contentHash,
    benchmarkPath: V16_PATH,
  });

  if (!postValidation.pass) {
    writeFileSync(
      resolve(OUT_DIR, "validation-v16-gold-policy-validation-failure-v2.json"),
      `${JSON.stringify({ preValidation, postValidation, removedViolations }, null, 2)}\n`,
    );
  ...
  const cert: GoldPolicyCertificateV2 = {
    ...
    note: "v1.5 stack — supersedes certificate-v1 (v1.4). Prior hash preserved in prePolicyValidation.",
    supersedes: {
      certificateFile: "validation-v16-gold-policy-certificate-v1.json",
      benchmarkHash: PRESERVED_HASH,
    },
  };

  writeFileSync(resolve(OUT_DIR, "validation-v16-gold-policy-certificate-v2.json"), `${JSON.stringify(cert, null, 2)}\n`);
  writeFileSync(
    resolve(OUT_DIR, "validation-v16-gold-policy-validation-report-v2.json"),
    `${JSON.stringify({ preValidation, postValidation, policyStack, removedViolations }, null, 2)}\n`,
  );
  writeFileSync(
    resolve(OUT_DIR, "validation-v16-gold-policy-correction-report-v2.json"),
    `${JSON.stringify(
      {
  ...
  writeFileSync(
    resolve(OUT_DIR, "validation-v16-gold-policy-stack-pin-v2.json"),
    `${JSON.stringify(
      {
        pinnedAt: cert.certifiedAt,
        policyStack,
        certificateFile: "validation-v16-gold-policy-certificate-v2.json",
        supersedes: "validation-v16-gold-policy-stack-pin-v1.json",
        immutableCertName: immutableBenchmarkFilename(SET_VERSION, "certified", correctedEnvelope.contentHash),
      },
      null,
      2,
    )}\n`,
    ...
    JSON.stringify(
      {
        ...
        certificate: "validation-v16-gold-policy-certificate-v2.json",
      },
      null,
      2,
    ),
