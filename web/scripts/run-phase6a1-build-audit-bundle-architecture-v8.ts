#!/usr/bin/env npx tsx
/** Build architecture review bundle archv8.zip — excludes authority-private roster generator source. */
...
const ZIP_PATH = resolve(OUT, "archv8.zip");
const STAGING = resolve(OUT, ".audit-bundle-archv8-staging");
const MANIFEST_NAME = "phase6a1-professor-plan-audit-bundle-archv8-manifest.json";
const BYTE_PIN_NAME = "phase6a1-professor-plan-audit-bundle-archv8-byte-pin.json";
...
async function main() {
  ...
  for (const f of [
    "phase6a1-professor-plan-prospective-pipeline-freeze-spec-v8.json",
    ...
    "phase6a1-professor-plan-v2-benchmark-disposition-v16-architecture.json",
    "phase6a1-professor-plan-post-gold-development-track-v16-architecture.json",
  ...
  for (const rel of [
    "web/scripts/run-phase6a1-seal-benchmark-architecture-v8.ts",
    "web/scripts/run-phase6a1-build-audit-bundle-architecture-v8.ts",
  ]) {
    stageFromRepo(rel, rel.replace(/\\/g, "/"), staged);
  }
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        version: "phase6a1-professor-plan-audit-bundle-archv8-manifest",
        generatedAt: GENERATED_AT,
        artifact: "archv8.zip",
        zipEntryCount: staged.length + 1,
        purpose: "Independent review of architecture amendment v8",
        supersedes: "archv7.zip",
        excludesAuthorityPrivateGenerator: true,
        includesCatalogVerifierScript: true,
        includesDeckResolutionTransitiveSources: true,
        frozenAmendmentV8ArchiveByteSha256: "9297688b07b85095672fedfb91f63669d71fc5e5d66ba319c10a654b0169cba1",
  ...
  const sealedManifestSha256 = sha256File(resolve(OUT, "phase6a1-professor-plan-benchmark-architecture-sealed-manifest-v8.json"));
  const bytePinPath = resolve(OUT, BYTE_PIN_NAME);
  writeFileSync(
    bytePinPath,
    JSON.stringify(
      {
        version: "phase6a1-professor-plan-audit-bundle-archv8-byte-pin",
        generatedAt: GENERATED_AT,
        artifact: "archv8.zip",
        byteSha256,
        manifestArtifact: MANIFEST_NAME,
        manifestSha256,
        sealedManifestV8Sha256: sealedManifestSha256,
        manifestIncludedInZip: true,
        uploadTogether: ["archv8.zip", BYTE_PIN_NAME],
      },
      null,
      2,
    ),
