/**
 * Validation v16 fresh selection — parser-blind gold, taxonomy v1.4, zero overlap with dev/v13/v14/v15/blind.
 * Sealed before RC6; emphasizes v15 forensic signal families without parser consultation.
 * Run: cd web && npx tsx scripts/create-validation-set-v16-fresh.ts
 */
...
const CANONICAL_PATH = "data/oracle-action-eval-validation-v16.json";
const CERTIFIER = "validation-set-v16-fresh-sealer-v150";
...
/** RC6-targeted families from v15 forensic census — card selection remains parser-blind. */
const V16_STRATA: V15Stratum[] = [
  {
...
function applyPolicyCorrection(
  ...
  return cases.map((c) => {
    ...
    return attachCaseScopeV11({
      ...
      goldReviewVersion: "validation-v16-gold-policy-corrector-v1",
      goldReviewedAt: new Date().toISOString(),
      parserConsulted: false,
      prePolicyValidation: {
        preservedContentHash: prePolicyHash,
        preservedArtifact: `data/milestones/validation-v16-certification/${immutableBenchmarkFilename(SET_VERSION, "prepolicy", prePolicyHash)}`,
        status: "superseded_before_parser_contact",
        correctedAt: new Date().toISOString(),
        corrector: "validation-v16-gold-policy-corrector-v1",
      },
...
async function main() {
  ...
  for (const stratum of V16_STRATA) {
    ...
    for (const card of picked) {
      cases.push(
        buildCatalogCase(
          card,
          `vh16-${String(cases.length + 1).padStart(4, "0")}`,
  ...
  assertNoOracleIdOverlap(cases, benchmarkExcluded, "validation v16 vs prior benchmarks");
  ...
  const prePolicyEnvelope: EvalDatasetEnvelope & { cases: CatalogEvalCase[]; goldCertification: Record<string, unknown> } = {
    setClassification: "validation_set_v16",
    evaluationSetVersion: SET_VERSION,
    taxonomyVersion: TAXONOMY_V14,
    contentHash: "",
    cases,
  ...
  if (!validation.pass) {
    writeFileSync(
      resolve(OUT_DIR, "validation-v16-gold-policy-validation-failure-v1.json"),
  ...
  const selectionManifest = {
    manifestVersion: "validation-v16-freeze-manifest-v150",
    frozenAt: reviewedAt,
    status: "SEALED",
    parserExecutionCount: 0,
    parserConsulted: false,
    ...
    note: "Fresh validation v16 — immutable prepolicy + certified copies preserved. parserExecutionCount=0 until RC6 authorization.",
  };
  const manifestHash = createHash("sha256").update(JSON.stringify(selectionManifest, null, 2)).digest("hex");
  writeFileSync(
    resolve(OUT_DIR, "validation-v16-freeze-manifest-v150.json"),
    `${JSON.stringify({ ...selectionManifest, contentHash: manifestHash }, null, 2)}\n`,
  );

  writeFileSync(
    resolve(OUT_DIR, "validation-v16-gold-policy-correction-report-v1.json"),
    `${JSON.stringify(
      {
  ...
  console.log(
    JSON.stringify(
      {
        validationV16Hash: certifiedEnvelope.contentHash,
        prePolicyHash: prePolicyEnvelope.contentHash,
        prepolicyImmutablePath,
        certifiedImmutablePath,
        manifestHash,
