#!/usr/bin/env npx tsx
/**
 * Phase 5.4.2 — Full 118-case human mechanical + retrieval review.
...
async function main() {
  ...
  const report = {
    version: "archetype-discovery-v1.4.2-human-mechanical-review",
  ...
  const outPath = resolve(outDir, "archetype-discovery-v1.4.2-human-mechanical-review.json");
  const matrixPath = resolve(outDir, "archetype-discovery-v1.4.1-to-v1.4.2-regression-matrix.json");
  writeFileSync(outPath, JSON.stringify(report, null, 2));
  writeFileSync(matrixPath, JSON.stringify({ version: "v1.4.1-to-v1.4.2", matrix: report.regressionMatrix, counts: regressionCounts, devGate542: report.devGate542 }, null, 2));

  console.log(JSON.stringify({ metrics: report.metrics, devGate542: report.devGate542, diagnosis: report.diagnosis, outPath, hash: createHash("sha256").update(JSON.stringify(report)).digest("hex") }, null, 2));
}
