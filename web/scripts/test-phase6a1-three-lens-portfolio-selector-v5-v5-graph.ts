...
const amendedV4Dir = resolve("data/milestones/deck-synthesis/phase6a1-professor-plan-experiment-v3-amended-v4/cases");

function loadCasePackages(caseId: string): SemanticPackage[] {
  const record = JSON.parse(readFileSync(resolve(amendedV4Dir, `${caseId}.json`), "utf8")) as {
...
for (const file of readdirSync(amendedV4Dir).filter((f) => f.endsWith(".json"))) {
