/**
 * Post-Stage-C parser freeze + v138 semantic gold closure before first parser run.
 * parserExecutionCount must remain 0 until eval-granted-v138-transfer-run-1.ts completes.
 */
...
async function main() {
  ...
  const envelopePath = resolve("data/oracle-action-eval-granted-nested-stage-c-v138.json");
  ...
  if (envelope.parserExecutionCount !== 0) {
    throw new Error(`v138 parserExecutionCount must be 0 before pre-run freeze (got ${envelope.parserExecutionCount})`);
  }
  ...
  for (const c of envelope.cases) {
    ...
    const benchmarkTargets = c.benchmarkTargets.map((t) => {
      const adjudication = buildSemanticAdjudication(t, oracleText);
      return { ...t, semanticAdjudication: adjudication };
    });
  ...
  assertAllBenchmarkIdentities(catalog, correctedCases, "v138-pre-run-identity");
  assertAllBenchmarkTargetValidities(catalog, correctedCases, "v138-pre-run-target");
  assertAllBenchmarkSemanticAdjudications(correctedCases, "v138-pre-run-semantic");
  ...
  const semanticGoldOnly = {
    ...
    checkpoint: "granted-nested-stage-c-v138-semantic-gold",
  ...
  writeFileSync(
    resolve("data/milestones/rc3-development/granted-v138-semantic-gold-freeze.json"),
  ...
  const grantedPaths = [
    ...
    "web/data/oracle-action-eval-granted-nested-stage-c-v138.json",
  ];
  ...
  const parserStateFreeze = {
    ...
    parentCheckpoint: "oracle-action-v1.37-rc3-granted-nested-complete",
    parserVersion: ORACLE_ACTION_RC3_PARSER_VERSION,
    ...
    semanticGoldHash,
    selectionManifestHash: fileHash("data/milestones/rc3-development/granted-nested-stage-c-v138-selection-manifest.json"),
  ...
  writeFileSync(
    resolve("data/milestones/rc3-development/granted-v138-parser-state-freeze.json"),
    `${JSON.stringify(parserStateFreeze, null, 2)}\n`,
  );

  const preRunReport = {
    generatedAt: new Date().toISOString(),
    checkpoint: "granted-v138-pre-run-closure",
    promotionBoundary,
    ...
    parserStateFreeze,
    parserExecutionCount: 0,
    authorizedNextStep: "single v138 transfer evaluation run (eval-granted-v138-transfer-run-1.ts)",
  };

  writeFileSync(
    resolve("data/milestones/rc3-development/granted-v138-pre-run-closure-report.json"),
