#!/usr/bin/env npx tsx
/**
 * Blind-v4 — single frozen v1.5.0 discovery run + human mechanical adjudication.
 * No tuning after outputs. Phase 6 remains WAIT until blind-v4 gate passes.
 */
...
import {
  ARCHETYPE_DISCOVERY_BLIND_HOLDOUT_V4,
  ARCHETYPE_DISCOVERY_BLIND_V4_STATUS,
  blindHoldoutV4CategoryComposition,
  blindHoldoutV4CommandZoneComposition,
  blindHoldoutV4SetHash,
} from "../src/lib/deck-synthesis/archetype-discovery-blind-holdout-v4";
import {
...
async function main() {
  ...
  const sealPath = resolve(process.cwd(), "data/milestones/deck-synthesis/archetype-discovery-blind-v4-seal-manifest.json");
  const sealManifest = JSON.parse(readFileSync(sealPath, "utf8")) as { blindV4Hash: string; status: string };

  if (sealManifest.status !== "SEALED") {
    throw new Error(`Blind-v4 seal manifest status must be SEALED, got ${sealManifest.status}`);
  }
  if (blindHoldoutV4SetHash() !== sealManifest.blindV4Hash) {
    throw new Error("Blind-v4 membership hash mismatch — aborting run.");
  }

  const preflight = benchmarkCommanderResolutionPreflight({
    catalog,
    commanderNames: ARCHETYPE_DISCOVERY_BLIND_HOLDOUT_V4.flatMap((c) => c.commanders),
  });
  ...
  const report = {
    version: "archetype-discovery-blind-v4-human-mechanical-review",
    ...
    freezeManifestVersion: "archetype-discovery-v1.5.0-freeze-manifest",
    blindV4Status: ARCHETYPE_DISCOVERY_BLIND_V4_STATUS,
    blindV4Hash: blindHoldoutV4SetHash(),
    sealManifestReference: "archetype-discovery-blind-v4-seal-manifest.json",
    generatedAt: new Date().toISOString(),
    interpretationNote:
      "Blind-v4 is the generalization test. DEV + blind-v1/v2/v3 spent results are regression evidence only — not generalization performance.",
    caseCount: records.length,
    scorableCases: scorable.length,
    accounting: benchmarkCaseAccounting(ARCHETYPE_DISCOVERY_BLIND_HOLDOUT_V4),
    categoryComposition: blindHoldoutV4CategoryComposition(),
    commandZoneComposition: blindHoldoutV4CommandZoneComposition(),
    ...
    blindV4Gate: gate,
    blindV4Verdict: gate.pass ? "PASS" : "FAIL",
    phase5ProductVerdict: gate.pass ? "PHASE_5_ACCEPTED_FOR_PRODUCT_CONSUMPTION" : "PHASE_5_BLOCKED_PENDING_ADJUDICATION",
    ...
    authorization: {
      phase55: "FROZEN",
      phase6: gate.pass ? "ELIGIBLE_FOR_AUTHORIZATION" : "WAIT",
      optimizer: "WAIT",
      professorImplementation: "WAIT",
      blindV4Retune: "PROHIBITED — would require blind-v5 or systematic family repair after human adjudication",
    },
    cases: records,
  };

  const outPath = resolve(process.cwd(), "data/milestones/deck-synthesis/archetype-discovery-blind-v4-human-mechanical-review.json");
  ...
  const updatedSeal = {
    ...
    evaluation: {
      ...
      humanReviewArtifact: "archetype-discovery-blind-v4-human-mechanical-review.json",
      humanReviewHash: hash,
      verdict: report.blindV4Verdict,
      phase5ProductVerdict: report.phase5ProductVerdict,
      statusAfterEvaluation: report.blindV4Verdict === "PASS" ? "EVALUATED_PASS" : "EVALUATED_FAIL",
      systematicFailureFamilies: systematicFamilies,
    },
  };
  if (report.blindV4Verdict === "FAIL") {
  ...
  console.log(
    JSON.stringify(
      {
        blindV4Verdict: report.blindV4Verdict,
        phase5ProductVerdict: report.phase5ProductVerdict,
        blindV4Gate: gate,
        abstentionQuality: abstentionCounts,
