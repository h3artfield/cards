#!/usr/bin/env npx tsx
/** Zada prospective Professor v3 smoke runner v1 — live NO_MODEL auth; full executable stack when separately authorized. */
...
import { runProfessorV3ZadaProspectiveSmokeExecutionV1 } from "./lib/phase6a1-professor-v3-zada-prospective-smoke-execution-v1";

loadProjectEnvLocal();

async function main() {
  const result = await runProfessorV3ZadaProspectiveSmokeExecutionV1({
  ...
  console.log(
    JSON.stringify(
      {
        version: "phase6a1-professor-v3-smoke-zada-prospective-executed-v1",
