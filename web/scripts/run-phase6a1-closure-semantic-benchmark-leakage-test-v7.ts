#!/usr/bin/env npx tsx
/** Leakage and integrity gates v7 for semantic closure benchmarks. */
...
function scanImplementationSealV7(): string[] {
  const findings: string[] = [];
  const sealPath = resolve(WEB_SCRIPTS, "run-phase6a1-closure-semantic-benchmark-seal-v7.ts");
...
function scanRepoForAuthorityLeak(snapshotDir: string): string[] {
  ...
  for (const f of readdirSync(snapshotDir).filter((x) => x.endsWith(".json") && x !== "manifest.json")) {
    ...
    if (raw.includes("holdout-v7-adjudication-decisions") || raw.includes("blinded-adjudication-sealed-v7")) {
...
export function writeLeakageReportV7(results: Record<string, { pass: boolean; findings: string[] }>): string {
  const path = resolve(OUT, "phase6a1-professor-plan-semantic-closure-leakage-test-report-v7.json");
  writeFileSync(path, JSON.stringify({
    version: "phase6a1-professor-plan-semantic-closure-leakage-test-report-v7",
    ...
    gates: [
      ...
      "authority_access_boundary_evidence_present",
    ],
...
export async function runFullLeakageSuiteV7(): Promise<{ pass: boolean; reportPath: string; results: Record<string, { pass: boolean; findings: string[] }> }> {
  const devDir = resolve(OUT, "phase6a1-professor-plan-dev36-semantic-closure-input-v6");
  const hoDir = resolve(OUT, "phase6a1-professor-plan-holdout-prospective-v7-semantic-closure-input-v7");
  ...
  const accessEvidencePath = resolve(OUT, "phase6a1-benchmark-authority-access-boundary-evidence-v1.json");

  const results = {
    dev36V6: await runCanonicalAndCoherenceGates(devDir, "dev36-v6", { strictHypothesisLinkage: false }),
    holdoutProspectiveV7: await runCanonicalAndCoherenceGates(hoDir, "holdout-prospective-v7"),
    ...
    implementationSealBoundary: { pass: scanImplementationSealV7().length === 0, findings: scanImplementationSealV7() },
    ...
    authorityPrivateGitignore: {
      pass: readFileSync(gitignorePath, "utf8").includes("benchmark-authority/**/private/"),
      findings: [],
    },
    authorityAccessBoundaryEvidence: {
      pass: existsSync(accessEvidencePath),
      findings: existsSync(accessEvidencePath) ? [] : ["missing phase6a1-benchmark-authority-access-boundary-evidence-v1.json"],
    },
  };
  const reportPath = writeLeakageReportV7(results);
  return { pass: Object.values(results).every((r) => r.pass), reportPath, results };
}

if (import.meta.url === `file://${process.argv[1]?.replace(/\\/g, "/")}` || process.argv[1]?.endsWith("run-phase6a1-closure-semantic-benchmark-leakage-test-v7.ts")) {
  runFullLeakageSuiteV7().then((r) => console.log(JSON.stringify(r, null, 2))).catch((e) => { console.error(e); process.exit(1); });
}
