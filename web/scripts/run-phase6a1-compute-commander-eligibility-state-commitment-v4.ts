/**
 * Compute commander eligibility-state commitment v2 from a single verified catalog snapshot (v4 writer, atomic write-once).
 */
...
async function main(): Promise<void> {
  if (!existsSync(PRE_ROSTER_VERIFICATION_PATH)) {
    throw new Error(
      `FAIL_CLOSED: missing pre-roster catalog verification artifact ${PRE_ROSTER_VERIFICATION_PATH}. Run run-phase6a1-verify-pinned-catalog-data-state-pre-roster-v3.ts first.`,
    );
  }
