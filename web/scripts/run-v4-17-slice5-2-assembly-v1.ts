/**
 * PROFESSOR v4.17 Slice 5.2 — run full deck assembly on saved Slice 3 live blueprints.
 */
...
const OUT_DIR = resolve(process.cwd(), "data/milestones/deck-synthesis/v4-17-slice5-2-assembly");
const OUT_DIR_ALT = resolve(process.cwd(), "web/data/milestones/deck-synthesis/v4-17-slice5-2-assembly");
...
async function main() {
  ...
  const out = outputDir();
  const caseFilter = process.env.PROFESSOR_V4_17_SLICE5_2_CASES?.split(",").map((s) => s.trim()).filter(Boolean);
  ...
  for (const caseId of cases) {
    ...
    writeFileSync(
      resolve(out, `${caseId}-slice5-2-report.json`),
      JSON.stringify(
        {
          caseId,
          success: assembly.success,
    ...
    console.log(
      `${caseId}: success=${assembly.success} failure=${assembly.failure} picks=${assembly.decisions.length} fdOpen=${openFunctionalDensityCountV417(assembly.blueprint)}`,
    );
  }

  writeFileSync(resolve(out, "slice5-2-summary.json"), JSON.stringify({ version: "professor-v4-17-slice5-2-assembly-v1", cases: results }, null, 2));
  console.log(`Wrote ${out}/slice5-2-summary.json`);
}
