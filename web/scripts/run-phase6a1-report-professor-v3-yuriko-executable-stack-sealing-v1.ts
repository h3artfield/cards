#!/usr/bin/env npx tsx
/** Report Yuriko executable-stack reseal bundle v1. */
...
const OUT_PATH = resolve(MILESTONES, "phase6a1-professor-v3-yuriko-executable-stack-sealing-report-v1.json");
const AUDIT_PATH = resolve(MILESTONES, "phase6a1-professor-v3-yuriko-executable-stack-sealing-audit-v1.json");
const MANIFEST_PATH = resolve(MILESTONES, "phase6a1-professor-v3-yuriko-executable-stack-sealing-v1-manifest.json");
const LIVE_ROOT_PATH = resolve(REPO, "web/scripts/lib/phase6a1-professor-v3-smoke-execution-authorization-live-root-v1.ts");
const OUT_ZIP_PATH = resolve(MILESTONES, "phase6a1-professor-v3-yuriko-executable-stack-sealing-v1.zip");
...
function main() {
  ...
  const report = {
    version: "phase6a1-professor-v3-yuriko-executable-stack-sealing-report-v1",
    ...
    yurikoProspectiveStackResealV1: audit.failed === 0 ? "PASS" : "FAIL",
    ...
    candidateAuthorization: PROFESSOR_V3_YURIKO_PROSPECTIVE_SMOKE_CANDIDATE_EXECUTION_AUTHORIZATION_V10,
    ...
    executionPinsArtifact: {
      path: PROFESSOR_V3_SMOKE_EXECUTION_PINS_V9_PATH,
      sha256: existsSync(PROFESSOR_V3_SMOKE_EXECUTION_PINS_V9_PATH) ? sha256File(PROFESSOR_V3_SMOKE_EXECUTION_PINS_V9_PATH) : null,
    },
    externalAnchors: {
      ...
      candidateAuthorizationDecision: PROFESSOR_V3_YURIKO_PROSPECTIVE_SMOKE_DECISION_V1,
    },
    instruction:
      "REPORT AND WAIT — Yuriko prospective stack reseal complete. Candidate auth is not live. Activation swaps external live-authorization root only.",
    nextAuthorizedStep:
      "Independent authorization to swap live authorization root to reviewed candidate values for one Yuriko prospective model smoke",
