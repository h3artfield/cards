#!/usr/bin/env npx tsx
/** Build architecture review bundle archv5.zip — excludes authority-private roster generator source. */
...
const ZIP_PATH = resolve(OUT, "archv5.zip");
const STAGING = resolve(OUT, ".audit-bundle-archv5-staging");
const MANIFEST_NAME = "phase6a1-professor-plan-audit-bundle-archv5-manifest.json";
const BYTE_PIN_NAME = "phase6a1-professor-plan-audit-bundle-archv5-byte-pin.json";
const GENERATED_AT = "2026-08-14T23:30:00.000Z";
...
async function main() {
  ...
  for (const f of [
    "phase6a1-professor-plan-prospective-pipeline-freeze-spec-v5.json",
    ...
    "phase6a1-professor-plan-authority-adjudication-disagreement-protocol-v3.json",
    "phase6a1-professor-plan-authority-adjudication-rubric-v3.json",
    ...
    "phase6a1-professor-plan-v2-benchmark-disposition-v13-architecture.json",
    "phase6a1-professor-plan-post-gold-development-track-v13-architecture.json",
  ...
  stageFromRepo("web/src/lib/deck-synthesis/benchmark-commander-eligibility-v1.ts", "web/src/lib/deck-synthesis/benchmark-commander-eligibility-v1.ts", staged);

  for (const rel of [
    "web/scripts/run-phase6a1-seal-benchmark-architecture-v5.ts",
    "web/scripts/run-phase6a1-build-audit-bundle-architecture-v5.ts",
  ]) {
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        version: "phase6a1-professor-plan-audit-bundle-archv5-manifest",
        generatedAt: GENERATED_AT,
        artifact: "archv5.zip",
        zipEntryCount: staged.length + 1,
        purpose: "Independent review of architecture amendment v5",
        supersedes: "archv4.zip",
  ...
  const sealedManifestSha256 = sha256File(resolve(OUT, "phase6a1-professor-plan-benchmark-architecture-sealed-manifest-v5.json"));
  const bytePinPath = resolve(OUT, BYTE_PIN_NAME);
  writeFileSync(
    bytePinPath,
    JSON.stringify(
      {
        version: "phase6a1-professor-plan-audit-bundle-archv5-byte-pin",
        generatedAt: GENERATED_AT,
        artifact: "archv5.zip",
        byteSha256,
        manifestArtifact: MANIFEST_NAME,
        manifestSha256,
        sealedManifestV5Sha256: sealedManifestSha256,
        manifestIncludedInZip: true,
        uploadTogether: ["archv5.zip", BYTE_PIN_NAME],
      },
      null,
      2,
    ),
