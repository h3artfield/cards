/**
 * Validation v17 fresh selection — parser-blind gold, taxonomy v1.4, Gold Policy v1.6.
 * Zero overlap with dev/v12–v16/blind/training/guardrails.
 * Card selection uses architectural-domain strata only — NOT v16 failure cards.
 * Run: cd web && npx tsx scripts/create-validation-set-v17-fresh.ts
 */
...
const CANONICAL_PATH = "data/oracle-action-eval-validation-v17.json";
const CERTIFIER = "validation-set-v17-fresh-sealer-v151";
...
/** Architectural-domain strata — parser-blind regex selection, independent of v16 mismatch cards. */
const V17_STRATA: V15Stratum[] = [
  {
...
function applyPolicyCorrection(
  ...
  return cases.map((c) => {
    ...
    return attachCaseScopeV11({
      ...
      goldReviewVersion: "validation-v17-gold-policy-corrector-v1",
      goldReviewedAt: new Date().toISOString(),
      parserConsulted: false,
      prePolicyValidation: {
        preservedContentHash: prePolicyHash,
        preservedArtifact: `data/milestones/validation-v17-certification/${immutableBenchmarkFilename(SET_VERSION, "prepolicy", prePolicyHash)}`,
        status: "superseded_before_parser_contact",
        correctedAt: new Date().toISOString(),
        corrector: "validation-v17-gold-policy-corrector-v1",
      },
...
async function main() {
  ...
  for (const stratum of V17_STRATA) {
    ...
    for (const card of picked) {
      cases.push(
        buildCatalogCase(
          card,
          `vh17-${String(cases.length + 1).padStart(4, "0")}`,
  ...
  assertNoOracleIdOverlap(cases, benchmarkExcluded, "validation v17 vs prior benchmarks");
  ...
  const prePolicyEnvelope: EvalDatasetEnvelope & { cases: CatalogEvalCase[]; goldCertification: Record<string, unknown> } = {
    setClassification: "validation_set_v17",
    evaluationSetVersion: SET_VERSION,
    taxonomyVersion: TAXONOMY_V14,
    contentHash: "",
    cases,
  ...
  if (!validation.pass) {
    writeFileSync(
      resolve(OUT_DIR, "validation-v17-gold-policy-validation-failure-v1.json"),
      `${JSON.stringify(validation, null, 2)}\n`,
    );
    throw new Error(`v17 gold policy validation failed: ${validation.violations.length} violations`);
  }
  ...
  const selectionManifest = {
    manifestVersion: "validation-v17-freeze-manifest-v151",
    frozenAt: reviewedAt,
    status: "SEALED",
    parserExecutionCount: 0,
    parserConsulted: false,
    ...
    note: "Fresh validation v17 — parser-blind architectural strata. parserExecutionCount=0; no parser contact.",
  };
  const manifestHash = createHash("sha256").update(JSON.stringify(selectionManifest, null, 2)).digest("hex");
  writeFileSync(
    resolve(OUT_DIR, "validation-v17-freeze-manifest-v151.json"),
    `${JSON.stringify({ ...selectionManifest, contentHash: manifestHash }, null, 2)}\n`,
  );

  writeFileSync(
    resolve(OUT_DIR, "validation-v17-gold-policy-correction-report-v1.json"),
    `${JSON.stringify(
      {
  ...
  console.log(
    JSON.stringify(
      {
        validationV17Hash: certifiedEnvelope.contentHash,
        prePolicyHash: prePolicyEnvelope.contentHash,
        prepolicyImmutablePath,
        certifiedImmutablePath,
        manifestHash,
