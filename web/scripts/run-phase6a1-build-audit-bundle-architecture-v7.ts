#!/usr/bin/env npx tsx
/** Build architecture review bundle archv7.zip — excludes authority-private roster generator source. */
...
const ZIP_PATH = resolve(OUT, "archv7.zip");
const STAGING = resolve(OUT, ".audit-bundle-archv7-staging");
const MANIFEST_NAME = "phase6a1-professor-plan-audit-bundle-archv7-manifest.json";
const BYTE_PIN_NAME = "phase6a1-professor-plan-audit-bundle-archv7-byte-pin.json";
const GENERATED_AT = "2026-08-15T00:30:00.000Z";
...
async function main() {
  ...
  for (const f of [
    "phase6a1-professor-plan-prospective-pipeline-freeze-spec-v7.json",
    ...
    "phase6a1-professor-plan-serialization-pilot-harmony-evidence-v1.json",
    "phase6a1-professor-plan-serialization-pilot-spec-v6.json",
    ...
    "phase6a1-professor-plan-authority-gold-clearance-policy-v1.json",
    ...
    "phase6a1-professor-plan-v2-benchmark-disposition-v15-architecture.json",
    "phase6a1-professor-plan-post-gold-development-track-v15-architecture.json",
  ...
  for (const rel of [
    "web/scripts/run-phase6a1-seal-benchmark-architecture-v7.ts",
    "web/scripts/run-phase6a1-build-audit-bundle-architecture-v7.ts",
  ]) {
    stageFromRepo(rel, rel.replace(/\\/g, "/"), staged);
  }
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        version: "phase6a1-professor-plan-audit-bundle-archv7-manifest",
        generatedAt: GENERATED_AT,
        artifact: "archv7.zip",
        zipEntryCount: staged.length + 1,
        purpose: "Independent review of architecture amendment v7",
        supersedes: "archv6.zip",
        excludesAuthorityPrivateGenerator: true,
        excludesDeterministicRosterSeed: true,
        includesFullCatalogDataArtifacts: true,
        includesDeckResolutionDependencySources: true,
  ...
  const sealedManifestSha256 = sha256File(resolve(OUT, "phase6a1-professor-plan-benchmark-architecture-sealed-manifest-v7.json"));
  const bytePinPath = resolve(OUT, BYTE_PIN_NAME);
  writeFileSync(
    bytePinPath,
    JSON.stringify(
      {
        version: "phase6a1-professor-plan-audit-bundle-archv7-byte-pin",
        generatedAt: GENERATED_AT,
        artifact: "archv7.zip",
        byteSha256,
        manifestArtifact: MANIFEST_NAME,
        manifestSha256,
        sealedManifestV7Sha256: sealedManifestSha256,
        manifestIncludedInZip: true,
        uploadTogether: ["archv7.zip", BYTE_PIN_NAME],
      },
      null,
      2,
    ),
