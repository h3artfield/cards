/**
 * Validation v15 fresh selection — parser-blind gold, taxonomy v1.4, zero overlap with dev/v13/v14/blind.
 * Emphasizes genuine v14 failure families for RC5 without parser consultation.
 * Run: cd web && npx tsx scripts/create-validation-set-v15-fresh.ts
 */
...
const SET_VERSION = "validation-v15";
const CANONICAL_PATH = "data/oracle-action-eval-validation-v15.json";
const CERTIFIER = "validation-set-v15-fresh-sealer-v150";
...
/** RC5-targeted families from v14 forensic census — card selection remains parser-blind. */
const V15_STRATA: V15Stratum[] = [
  {
...
function applyPolicyCorrection(
  ...
  return cases.map((c) => {
    ...
    return attachCaseScopeV11({
      ...
      goldReviewVersion: "validation-v15-gold-policy-corrector-v1",
      goldReviewedAt: new Date().toISOString(),
      parserConsulted: false,
      prePolicyValidation: {
        preservedContentHash: prePolicyHash,
        preservedArtifact: `data/milestones/validation-v15-certification/${immutableBenchmarkFilename(SET_VERSION, "prepolicy", prePolicyHash)}`,
        status: "superseded_before_parser_contact",
        correctedAt: new Date().toISOString(),
        corrector: "validation-v15-gold-policy-corrector-v1",
      },
...
async function main() {
  ...
  for (const stratum of V15_STRATA) {
    ...
    for (const card of picked) {
      cases.push(
        buildCatalogCase(
          card,
          `vh15-${String(cases.length + 1).padStart(4, "0")}`,
  ...
  assertNoOracleIdOverlap(cases, benchmarkExcluded, "validation v15 vs prior benchmarks");
  ...
  const prePolicyEnvelope: EvalDatasetEnvelope & { cases: CatalogEvalCase[]; goldCertification: Record<string, unknown> } = {
    setClassification: "validation_set_v15",
    evaluationSetVersion: SET_VERSION,
    taxonomyVersion: TAXONOMY_V14,
    contentHash: "",
    cases,
  ...
  if (!validation.pass) {
    writeFileSync(
      resolve(OUT_DIR, "validation-v15-gold-policy-validation-failure-v1.json"),
      `${JSON.stringify(validation, null, 2)}\n`,
    );
    throw new Error(`v15 gold policy validation failed: ${validation.violations.length} violations`);
  }
  ...
  const selectionManifest = {
    manifestVersion: "validation-v15-freeze-manifest-v150",
    frozenAt: reviewedAt,
    status: "SEALED",
    parserExecutionCount: 0,
    parserConsulted: false,
    ...
    note: "Fresh validation v15 — immutable prepolicy + certified copies preserved. parserExecutionCount=0 until RC5 authorization.",
  };
  const manifestHash = createHash("sha256").update(JSON.stringify(selectionManifest, null, 2)).digest("hex");
  writeFileSync(
    resolve(OUT_DIR, "validation-v15-freeze-manifest-v150.json"),
    `${JSON.stringify({ ...selectionManifest, contentHash: manifestHash }, null, 2)}\n`,
  );

  writeFileSync(
    resolve(OUT_DIR, "validation-v15-gold-policy-correction-report-v1.json"),
    `${JSON.stringify(
      {
  ...
  console.log(
    JSON.stringify(
      {
        validationV15Hash: certifiedEnvelope.contentHash,
        prePolicyHash: prePolicyEnvelope.contentHash,
        prepolicyImmutablePath,
        certifiedImmutablePath,
        manifestHash,
