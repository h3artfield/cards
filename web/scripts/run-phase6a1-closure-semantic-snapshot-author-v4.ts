#!/usr/bin/env npx tsx
/** Semantic runtime snapshot author v4 — reads fixture catalog ONLY. */
...
function fixtureToRuntimeSnapshot(fixture: CaseSemanticFixture) {
  const snapshot = {
    ...
    runtimeSchemaVersion: "phase6a1-semantic-closure-runtime-input-v4",
...
export function authorSemanticSnapshotsV4(
  manifestRelPath: string,
  catalogDir: string,
  snapshotDir: string,
  manifestVersion: string,
): { snapshotDir: string; manifestSha256: string } {
  const manifest = JSON.parse(readFileSync(join(catalogDir, manifestRelPath), "utf8")) as {
...
export function authorDev36SemanticSnapshotsV4(): { snapshotDir: string; manifestSha256: string } {
  const catalogDir = resolve(OUT, "phase6a1-professor-plan-semantic-fixture-catalog-v4");
  return authorSemanticSnapshotsV4(
    "dev36-v4-manifest.json",
    catalogDir,
    resolve(OUT, "phase6a1-professor-plan-dev36-semantic-closure-input-v4"),
    "phase6a1-professor-plan-dev36-semantic-closure-input-v4",
  );
}

if (import.meta.url === `file://${process.argv[1]?.replace(/\\/g, "/")}` || process.argv[1]?.endsWith("run-phase6a1-closure-semantic-snapshot-author-v4.ts")) {
  authorDev36SemanticSnapshotsV4();
  console.log("dev36-v4 semantic snapshots written");
}
