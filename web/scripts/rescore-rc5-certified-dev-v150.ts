/**
 * RC5 certified development rescore — combined + unrelated catalog + invariants.
 * Run: cd web && npx tsx scripts/rescore-rc5-certified-dev-v150.ts
 */
...
function main() {
  ...
  const goldPolicyValidation = validateBenchmarkGoldPolicy({
    ...
    benchmarkPath: "rc5-certified-development-with-migration-overlay",
  });
  ...
  const remainingKnownFn = v14SpentRegression.action.rows.filter((r) => r.metrics.fn > 0);
  const recoveredGenuineFn = v14SpentRegression.action.rows.filter(
    (r) => RECOVERED_FN_CASES.includes(r.sourceV14CaseId) && r.metrics.fn === 0,
  );
  const fixedGenuineFp = v14SpentRegression.fp.rows.filter((r) => r.pass);
  const remainingGenuineFp = v14SpentRegression.fp.rows.filter((r) => !r.pass);
  ...
  const report = {
    ...
    unrelatedCatalogPositive: {
      ...
      note: "Certified unrelated-catalog generalization gate — distinct from v14 spent regression-pack recall",
    },
    v14SpentRegressionPack: {
      actionScore: v14SpentRegression.action.accepted,
      fpScore: v14SpentRegression.fp,
      integrity: v14SpentRegression.integrity,
      leakage: v14SpentRegression.leakage,
      recoveredGenuineFn: recoveredGenuineFn.map((r) => ({
        sourceV14CaseId: r.sourceV14CaseId,
        cardName: r.cardName,
        tp: r.metrics.tp,
      })),
      remainingKnownGenuineFn: remainingKnownFn.map((r) => ({
        sourceV14CaseId: r.sourceV14CaseId,
        cardName: r.cardName,
        fn: r.metrics.fn,
        targets: r.targets.map((t) => t.actionType),
      })),
      fixedGenuineFp: fixedGenuineFp.map((r) => ({
        sourceV14CaseId: r.sourceV14CaseId,
        cardName: r.cardName,
      })),
      remainingGenuineFp: remainingGenuineFp.map((r) => ({
        sourceV14CaseId: r.sourceV14CaseId,
        cardName: r.cardName,
        observedAction: r.falsePositiveTarget.observedAction,
      })),
      regressionPackPath: "data/oracle-action-eval-rc5-v14-regression-v151.json",
      note: "Adjudicated v14 FN/FP ledger on spent material (v151 gold-corrected) — informational, not a gate",
    },
    policyInvariants,
    invariantsPass,
    pass1Closed:
  ...
  const outPath = resolve("data/milestones/rc5-development/rc5-certified-dev-rescore-v150.json");
