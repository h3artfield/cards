...
export function parseAbilityBlock(input: {
  ...
  const modalOption = isModalOptionAbility({
    modalOptionId: input.modalOptionId,
    paragraphText: input.paragraphText,
  });
  const optionBody = modalOption ? normalizeModalOptionBody(input.paragraphText) : undefined;
  const clauseParagraph = optionBody?.bodyText ?? input.paragraphText;
  const clauseOffset = optionBody?.bodyLocalStart ?? 0;

  const clauses = compoundClauseSpansWithRoles(clauseParagraph, input.abilityId).map((span, idx) => ({
    clauseId: `${input.abilityId}:clause-${idx}`,
    role: span.role,
    text: span.text,
    localStart: clauseOffset + span.localStart,
    localEnd: clauseOffset + span.localStart + span.text.length,
  }));
