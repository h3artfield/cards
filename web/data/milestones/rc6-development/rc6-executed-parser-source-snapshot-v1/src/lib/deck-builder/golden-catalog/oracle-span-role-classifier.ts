...
/** Stun-counter rules reminder — structural If/would/instead, not a host replacement ability. */
function isCounterRulesReminderParenthetical(inner: string): boolean {
  const t = inner.trim();
  return (
    /^If a permanent with a stun counter would become untapped,\s*remove one from it instead\.?$/i.test(t) ||
    /^If a permanent with a stun counter would become untapped,\s*remove a stun counter from it instead\.?$/i.test(t)
  );
}

/** Segment text with mechanic/reminder parentheticals removed for role analysis. */
function roleAnalysisTextExcludingReminders(paragraph: string, segStart: number, segEnd: number): string {
  let slice = paragraph.slice(segStart, segEnd);
  const reminders = findReminderSpans(paragraph).filter(
    (r) => r.localStart >= segStart && r.localEnd <= segEnd,
  );
  if (reminders.length === 0) return slice.trim();
  let result = slice;
  for (const r of [...reminders].sort((a, b) => b.localStart - a.localStart)) {
    const relStart = r.localStart - segStart;
    const relEnd = r.localEnd - segStart;
    result = `${result.slice(0, relStart).trimEnd()} ${result.slice(relEnd).trimStart()}`.trim();
  }
  return result.trim();
}
...
export function findReminderSpans(paragraph: string): TextSpanRole[] {
  ...
  for (let i = 0; i < paragraph.length; i++) {
    ...
    if (
      /\b(?:Storm|Forecast|Cascade|Annihilator|Landfall|Morbid|Heroic|Constellation|Ninjutsu|Equip|Reconfigure|Bestow|Fortify|Crew|Embalm|Eternalize|Mutate|Flashback|Spectacle|Harmonize|Disturb|Warp|Blitz|Cleave|Investigate)\s*[\{(]?[^()]*$/i.test(
        prefix,
      )
    ) {
      spans.push({
    ...
    if (
      /^Create a (?:Clue|Treasure|Blood|Food) token\.\s*/i.test(lead) ||
      /^They create a (?:Clue|Treasure|Blood|Food) token\.?\s*$/i.test(lead)
    ) {
    ...
    if (isCounterRulesReminderParenthetical(inner)) {
      spans.push({
        role: "mechanic_reminder",
        localStart: i,
        localEnd: close,
...
/** True when evidence at localStart derives from a classified mechanic/reminder parenthetical span. */
export function isEvidenceInReminderSpan(
  paragraph: string,
  localStart: number,
  localEnd?: number,
): boolean {
  const end = localEnd ?? localStart + 1;
  return findReminderSpans(paragraph).some(
    (r) =>
      (r.role === "reminder_text" || r.role === "mechanic_reminder") &&
...
function replacementEventEnd(paragraph: string): number | null {
  const re = /\bIf (?:a |an |target |you |each |that )[^,]+ would [^,]+,\s*/gi;
  let m: RegExpExecArray | null;
  while ((m = re.exec(paragraph)) !== null) {
    if (m.index === undefined) continue;
    const matchStart = m.index;
    const openParen = paragraph.lastIndexOf("(", matchStart);
    const closeBefore = paragraph.lastIndexOf(")", matchStart);
    if (openParen > closeBefore && openParen >= 0) {
      const close = findMatchingCloseParen(paragraph, openParen);
      const inner = paragraph.slice(openParen + 1, close - 1).trim();
      if (isCounterRulesReminderParenthetical(inner)) continue;
    }
    const reminders = findReminderSpans(paragraph);
    if (reminders.some((r) => matchStart >= r.localStart && matchStart < r.localEnd)) continue;
    return matchStart + m[0].length;
  }
  return null;
}
...
export function classifyTextRoleAt(input: {
  ...
  /** When set, replacement/trigger/cost detection uses this segment (minus isolated reminders). */
  ...
  const segmentBounds = input.segmentBounds;
  const roleSegmentText = segmentBounds
    ? roleAnalysisTextExcludingReminders(paragraph, segmentBounds.start, segmentBounds.end)
    : null;
  const { clauseStart, clauseText } = clauseAtPosition(paragraph, adjustedStart);
  const roleText = roleSegmentText && roleSegmentText.length >= 2 ? roleSegmentText : clauseText;
  const roleTextOffset = segmentBounds?.start ?? clauseStart;
  const clauseLocalStart = adjustedStart - roleTextOffset;
  const clauseLocalEnd = adjustedEnd - roleTextOffset;
  ...
  const trigEnd = triggerEventEnd(roleText);
  if (trigEnd !== null && clauseLocalStart < trigEnd) return "trigger_event";

  const colon = activatedColonSplit(roleText);
  if (colon) {
  ...
  const ifYouDo = ifYouDoPhraseBounds(roleText);
  if (ifYouDo) {
...
export function isPersistentZoneCastPermission(evidenceText: string): boolean {
  ...
  return (
    /\bcast(?: [\w'-]+)* spells from (?:your )?(?:hand|graveyard|library|exile)\b/i.test(evidenceText) ||
...
export function isOneShotCastPermission(paragraph: string, localStart: number, evidenceText: string): boolean {
  ...
  if (/\b(?:You may cast (?:it|that card|that spell|the copy|a copy of its spell)[^.\n]*(?:without paying|\.|$))/i.test(evidenceText)) {
    return true;
  }
  if (/\byou may cast a copy of its spell\b/i.test(evidenceText)) return true;
